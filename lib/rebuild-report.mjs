...
function startTier2(body) {
  // Immediately show/create log panel
  var logPanel = document.getElementById('tier2-log');
  if (!logPanel) {
    logPanel = document.createElement('div');
    logPanel.id = 'tier2-log';
    logPanel.className = 'tier2-log-panel';
    var nav = document.querySelector('.nav');
    if (nav) nav.after(logPanel);
  }
  logPanel.style.display = 'block';
  logPanel.textContent = '⏳ Starting Tier 2 analysis...\\n';
  ...
  .then(r => r.json())
  .then(data => {
    if (data.error) { logPanel.textContent += '❌ Error: ' + data.error + '\\n'; return; }
    logPanel.textContent += '✅ Started — polling for updates...\\n';
    pollTier2();
  })
  .catch(e => { logPanel.textContent += '❌ Failed: ' + e.message + '\\n'; });
}
      ...
      .then(data => {
        if (data.lines && data.lines.length > 0) {
          ...
          if (logPanel) {
            ...
            // Keep panel visible — auto-scroll if user is near bottom
            var nearBottom = logPanel.scrollHeight - logPanel.scrollTop - logPanel.clientHeight < 100;
