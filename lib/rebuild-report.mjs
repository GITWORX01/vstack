...
for (let i = 0; i < pageScenes.length; i++) {
  ...
  for (const shot of (scene.shots || [])) {
    ...
    scenesHtml += `<div class="shot" data-sc="${scene.sceneNumber}" data-sh="${shot.shotNumber}" data-start="${startSec.toFixed(3)}" data-end="${endSec.toFixed(3)}"><div class="shot-scene-wm">Scene ${scene.sceneNumber}</div><label class="shot-check"><input type="checkbox" class="shot-cb" onchange="updateJoinBtn()" value="${scene.sceneNumber}-${shot.shotNumber}"></label><div class="shot-num-label">Shot ${shot.shotNumber}</div><div class="fw" ${playOnclick}><img src="frames/${shot._frameFirst || ''}?v=${buildTs}" loading="lazy" /><div class="fl fg">▶ ${formatTs(startSec)}</div></div>
...
.shot-num-label{grid-column:2/4;padding:4px 8px;font-size:14px;font-weight:bold;color:#4fc3f7;letter-spacing:1px}
...
<span class="join-count" id="join-count" style="color:#ff9800;font-size:12px;margin-left:8px"></span>
<button class="unselect-btn" id="unselect-btn" onclick="unselectAll()" style="display:none;background:#333;color:#eee;border:none;padding:5px 10px;border-radius:4px;cursor:pointer;font-size:11px">Unselect All</button>
<button class="join-btn" id="join-btn" onclick="joinShots()" style="display:none;background:#e94560;color:#fff;border:none;padding:5px 10px;border-radius:4px;cursor:pointer;font-size:11px;font-weight:bold">⊕ Join Selected</button>
</div>
