...
const scenesPerPage = options.scenesPerPage || 30;
const totalSceneCount = allScenes.length;
const totalPages = Math.ceil(totalSceneCount / scenesPerPage);
const currentPage = Math.min(Math.max(1, page), totalPages || 1);
const startIdx = (currentPage - 1) * scenesPerPage;
const endIdx = Math.min(startIdx + scenesPerPage, totalSceneCount);
...
function editCharacters(el) {
  ...
  function save() {
    ...
    // Update data-characters on scene div so speaker dropdowns immediately reflect changes
    sceneDiv.dataset.characters = JSON.stringify(newChars);

    // Save to scenes.json (use cache if available to avoid full JSON parse)
