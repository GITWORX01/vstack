...
function editCharacters(el) {
  ...
  function save() {
    ...
    var scenes = window._scenesCache;
    if (!scenes) { console.error('No scenes cache'); return; }
...
function editDialogueText(el, sceneNumber, shotNum, dlIdx) {
  ...
  function save() {
    ...
    fetch('/api/save-json/' + encodeURIComponent(episodeId), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ json: JSON.stringify(scenes, null, 2) })
