...
for (let i = 0; i < pageScenes.length; i++) {
  ...
  for (const shot of (scene.shots || [])) {
    ...
    scenesHtml += `<div class="shot" data-sc="${scene.sceneNumber}" data-sh="${shot.shotNumber}" data-start="${startSec.toFixed(3)}" data-end="${endSec.toFixed(3)}"><label class="shot-check"><input type="checkbox" class="shot-cb" onchange="updateJoinBtn()" value="${scene.sceneNumber}-${shot.shotNumber}"></label><div class="fw" ${playOnclick}><img src="frames/${shot._frameFirst || ''}" loading="lazy" /><div class="fl fg">▶ ${formatTs(startSec)}</div></div>
      <div class="fw" ${playOnclick}><img src="frames/${shot._frameLast || ''}" loading="lazy" /><div class="fl fr">◼ ${formatTs(endSec)}</div></div>
