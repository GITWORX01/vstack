...
function editCharacters(el) {
  ...
  // Pre-fetch scenes in background so save is instant
  if (!window._scenesCache) {
  ...
  input.style.cssText = 'width:320px;padding:4px 6px;background:#0a0a1a;border:1px solid #4fc3f7;color:#eee;border-radius:3px;font-size:12px';
  input.placeholder = 'Character 1, Character 2, ...';

  el.textContent = '';
  el.appendChild(input);
  // Defer focus to next frame to avoid layout thrash
  ...
  function save() {
    ...
    if (newChars.length === 0 || input.value.trim() === currentChars.join(', ')) {
  ...
  input.onblur = function() { setTimeout(save, 150); };
}
