...
function closeFieldEditor() {
  if (activeFieldEditor) { activeFieldEditor.remove(); activeFieldEditor = null; }
}
document.addEventListener('mousedown', function(e) {
  if (activeFieldEditor && !activeFieldEditor.contains(e.target)) closeFieldEditor();
});
...
function editSceneField(el, sceneNum, fieldName) {
  ...
  hint.textContent = (isLong ? 'Ctrl+Enter' : 'Enter') + ' to save · Escape to cancel';
  ...
  input.onkeydown = function(e) {
    ...
    if (e.key === 'Escape') { e.preventDefault(); closeFieldEditor(); }
    e.stopPropagation();
  };
