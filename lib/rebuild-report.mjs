...
function analyzeTier2(sceneNum, shotNum) {
  ...
  modal.querySelector('#t2-srt-browse').onclick = function() {
    fetch('/api/browse-files?dir=' + encodeURIComponent('C:\\\\') + '&ext=srt')
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (data.files && data.files.length > 0) {
          var picker = document.createElement('div');
          picker.style.cssText = 'background:#0a0a1a;border:1px solid #444;border-radius:4px;max-height:200px;overflow-y:auto;margin-top:4px';
          data.files.forEach(function(f) {
            ...
            row.onclick = function() {
              modal.querySelector('#t2-srt').value = f.path || f;
              picker.remove();
            };
            picker.appendChild(row);
          });
          modal.querySelector('#t2-srt').parentNode.appendChild(picker);
        }
      });
  };
