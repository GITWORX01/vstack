...
export async function generateReportHtml(episodeId, videoPath, options = {}) {
const EPISODE_ID = episodeId;
const LOCAL_FILE = videoPath;
...
// Pagination — adaptive: cap at ~150 shots per page for responsiveness
const page = options.page || 1;
const MAX_SHOTS_PER_PAGE = 200;
const requestedScenesPerPage = options.scenesPerPage || 30;
const totalSceneCount = allScenes.length;

// Build page boundaries based on shot count
const pageBreaks = [0]; // start indices of each page
let shotAcc = 0;
for (let si = 0; si < allScenes.length; si++) {
  shotAcc += (allScenes[si].shots?.length || 1);
  if (shotAcc >= MAX_SHOTS_PER_PAGE && si + 1 < allScenes.length) {
    pageBreaks.push(si + 1);
    shotAcc = 0;
  }
}
const totalPages = pageBreaks.length;
const currentPage = Math.min(Math.max(1, page), totalPages);
const startIdx = pageBreaks[currentPage - 1];
const endIdx = currentPage < totalPages ? pageBreaks[currentPage] : totalSceneCount;
const pageScenes = allScenes.slice(startIdx, endIdx);

// Compute totals from ALL scenes (for stats bar)
const totalShotsAll = allScenes.reduce((s, sc) => s + (sc.shots?.length || 0), 0);

const buildTs = Date.now(); // cache-buster for frame images

console.log(`Rebuilding report for ${EPISODE_ID}: ${allScenes.length} scenes, ${totalShots} shots (page ${currentPage}/${totalPages})`);

// Build scenes HTML
const sceneColors = ['#4fc3f7', '#e94560', '#a5d6a7', '#ff9800', '#ce93d8', '#ffcc80', '#80cbc4', '#ef5350', '#81d4fa', '#ffab91', '#b39ddb', '#c5e1a5'];
let scenesHtml = '';
for (let i = 0; i < pageScenes.length; i++) {
  ...
  scenesHtml += `<div class="scene${creditsClass}" style="border-left-color:${borderColor}" data-characters="${charsJson}"><div class="scene-hdr"><h2>Scene ${scene.sceneNumber}${creditsBadge}</h2>
    ...
    <span class="pill loc" ondblclick="editSceneField(this,${scene.sceneNumber},'location')">${scene.location || '?'}</span>
    <span class="pill mood" ondblclick="editSceneField(this,${scene.sceneNumber},'mood')">${scene.mood || '?'}</span>
    <span class="pill chr" ondblclick="editCharacters(this)" data-episode="${EPISODE_ID}" data-scene="${scene.sceneNumber}">${(scene.characters || []).join(', ')}</span></div>
    <div class="plot" ondblclick="editSceneField(this,${scene.sceneNumber},'plotSignificance')" style="cursor:pointer" title="Double-click to edit">${scene.plotSignificance || ''}</div>
    <div class="scene-details">
      ${scene.lighting ? `<div class="sd"><span class="sd-label">Lighting</span> <span ondblclick="editSceneField(this,${scene.sceneNumber},'lighting')" style="cursor:pointer">${scene.lighting}</span></div>` : ''}
      ${scene.music ? `<div class="sd"><span class="sd-label">Music</span> <span ondblclick="editSceneField(this,${scene.sceneNumber},'music')" style="cursor:pointer">${scene.music}</span></div>` : ''}
      ${scene.costuming ? `<div class="sd"><span class="sd-label">Costumes</span> <span ondblclick="editSceneField(this,${scene.sceneNumber},'costuming')" style="cursor:pointer">${scene.costuming}</span></div>` : ''}
      ${(scene.tags || []).length ? `<div class="sd"><span class="sd-label">Tags</span> <span class="scene-tags" ondblclick="editSceneField(this,${scene.sceneNumber},'tags')" style="cursor:pointer">${scene.tags.map(t => '<span class="tag">' + t + '</span>').join(' ')}</span></div>` : ''}
      ${(scene.supercutPotential || []).length ? `<div class="sd"><span class="sd-label">Supercut</span> <span class="scene-tags" ondblclick="editSceneField(this,${scene.sceneNumber},'supercutPotential')" style="cursor:pointer">${scene.supercutPotential.map(t => '<span class="stag">' + t + '</span>').join(' ')}</span></div>` : ''}
    </div>
    <div class="tier2-controls">
      <button class="shot-edit-btn" onclick="editSceneJson(${scene.sceneNumber})">{ } Edit Scene</button>
      <button class="tier2-scene-btn" onclick="reanalyzeScene(${scene.sceneNumber})" title="Re-run Tier 1 scene detection on this scene's video segment">&#128260; Tier 1: Re-analyze</button>
      <button class="tier2-scene-btn" onclick="analyzeTier2(${scene.sceneNumber})" title="Tier 2: Enrich all shots in this scene">&#9889; Tier 2: Analyze Scene ${scene.sceneNumber}</button>
      <button class="shot-edit-btn" onclick="regenSceneFrames(${scene.sceneNumber})" title="Re-extract all frame thumbnails for this scene from the video">&#127910; Regen Frames</button>
  ...
  for (const shot of (scene.shots || [])) {
    ...
    scenesHtml += `<div class="shot" data-sc="${scene.sceneNumber}" data-sh="${shot.shotNumber}" data-start="${startSec.toFixed(3)}" data-end="${endSec.toFixed(3)}"><div class="shot-scene-wm">Scene ${scene.sceneNumber}</div><label class="shot-check"><input type="checkbox" class="shot-cb" onchange="updateJoinBtn()" value="${scene.sceneNumber}-${shot.shotNumber}"></label><div class="fw" ${playOnclick}><img src="frames/${shot._frameFirst || ''}?v=${buildTs}" loading="lazy" /><div class="fl fg">▶ ${formatTs(startSec)}</div></div>
      <div class="fw" ${playOnclick}><img src="frames/${shot._frameLast || ''}?v=${buildTs}" loading="lazy" /><div class="fl fr">◼ ${formatTs(endSec)}</div></div>
      ...
      <div class="f"><span class="fl2">Subject</span><div class="subj" ondblclick="editShotField(this,${scene.sceneNumber},${shot.shotNumber},'subject')" style="cursor:pointer">${shot.subject || '?'}</div></div>
      <div class="f"><span class="fl2">Action</span><div ondblclick="editShotField(this,${scene.sceneNumber},${shot.shotNumber},'action')" style="cursor:pointer">${shot.action || '?'}</div></div>
      ${shot.cameraMovement ? `<div class="f"><span class="fl2">Camera</span><div class="cam" ondblclick="editShotField(this,${scene.sceneNumber},${shot.shotNumber},'cameraMovement')" style="cursor:pointer">${shot.cameraMovement}</div></div>` : ''}
      ${exprs ? `<div class="f"><span class="fl2">Expressions</span><div ondblclick="editShotField(this,${scene.sceneNumber},${shot.shotNumber},'characterExpressions')" style="cursor:pointer">${exprs}</div></div>` : ''}
      ${shot.dialogue?.length ? `<div class="f dialogue"><span class="fl2">Dialogue</span><div class="dialogue-lines" data-episode="${EPISODE_ID}" data-scene="${scene.sceneNumber}" data-shot="${shot.shotNumber}">${shot.dialogue.map((d, dIdx) => {
        ...
        return `<div class="dl" data-dlidx="${dIdx}">${moveUp}${moveDown}${ts}${speaker}<span class="dl-text" ondblclick="editDialogueText(this, ${scene.sceneNumber}, ${shot.shotNumber}, ${dIdx})">"${textEsc}"</span><button class="dl-clone" onclick="cloneDialogueLine(this, ${scene.sceneNumber}, ${shot.shotNumber}, ${dIdx})" title="Clone line">⧉</button><button class="dl-delete" onclick="deleteDialogueLine(this, ${scene.sceneNumber}, ${shot.shotNumber}, ${dIdx})" title="Delete line">&times;</button></div>`;
      }).join('')}</div></div>` : ''}
      ...
      ${(shot.supercutPotential || []).length ? '<div class="tgs" ondblclick="editShotField(this,' + scene.sceneNumber + ',' + shot.shotNumber + ',\'supercutPotential\')" style="cursor:pointer">' + shot.supercutPotential.map(t => '<span class="stag">' + t + '</span>').join('') + '</div>' : ''}
      ...
      ${shot._tier !== 2 ? `<button class="shot-analyze-btn" onclick="analyzeTier2(${scene.sceneNumber},${shot.shotNumber})" title="Tier 2: Enrich shot metadata">&#9889; Tier 2</button>` : ''}
      </div></div>`;
    // Split Scene button between shots (not after the last shot)
    const isLastShot = shot === (scene.shots || [])[scene.shots.length - 1];
    if (!isLastShot) {
      scenesHtml += `<div class="split-divider"><button class="split-btn" onclick="splitScene(${scene.sceneNumber},${shot.shotNumber})" title="Split this scene into two at this point">✂ Split Scene</button></div>`;
    }
  }
...
let paginationHtml = '';
if (totalPages > 1) {
  const pageBaseUrl = '/api/report-html/' + EPISODE_ID;
  let pageButtons = '';
  for (let p = 1; p <= totalPages; p++) {
    const active = p === currentPage ? 'background:#4fc3f7;color:#000;font-weight:bold' : 'background:#1a1a2e;color:#aaa';
    pageButtons += `<a href="${pageBaseUrl}?page=${p}" style="${active};padding:6px 12px;border-radius:4px;text-decoration:none;font-size:13px">${p}</a> `;
  }
  const prevLink = currentPage > 1 ? `<a href="${pageBaseUrl}?page=${currentPage - 1}" style="color:#4fc3f7;text-decoration:none;font-size:13px">◀ Prev</a>` : `<span style="color:#333;font-size:13px">◀ Prev</span>`;
  const nextLink = currentPage < totalPages ? `<a href="${pageBaseUrl}?page=${currentPage + 1}" style="color:#4fc3f7;text-decoration:none;font-size:13px">Next ▶</a>` : `<span style="color:#333;font-size:13px">Next ▶</span>`;

  paginationHtml = `<div style="display:flex;align-items:center;justify-content:center;gap:12px;padding:12px 0;border-bottom:1px solid #222;margin-bottom:16px">
    ${prevLink} ${pageButtons} ${nextLink}
    <span style="color:#888;font-size:12px;margin-left:12px">Page ${currentPage} of ${totalPages} — Scenes ${startIdx + 1}–${endIdx} of ${totalSceneCount}</span>
  </div>`;
}

const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><base href="/${EPISODE_ID}/"><title>${EPISODE_ID} — Scene Review Report</title>
...
.shot{display:grid;grid-template-columns:24px 160px 160px 1fr;gap:0;background:#1a1a2e;border-radius:8px;overflow:hidden;border-left:3px solid #333;position:relative}
.shot-scene-wm{position:absolute;bottom:6px;right:10px;font-size:20px;font-weight:900;color:rgba(255,255,255,0.06);pointer-events:none;z-index:1;text-transform:uppercase;letter-spacing:2px}
...
.split-divider{display:flex;align-items:center;justify-content:center;padding:2px 0;opacity:0.3;transition:opacity 0.2s}
.split-divider:hover{opacity:1}
.split-btn{background:none;border:1px dashed #666;color:#888;padding:2px 12px;border-radius:3px;cursor:pointer;font-size:10px;width:100%}
.split-btn:hover{border-color:#e94560;color:#e94560;background:rgba(233,69,96,0.1)}
...
.split-divider.search-hidden{display:none !important}
...
.shot-editor-modal-overlay{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.9);z-index:200;display:flex;align-items:center;justify-content:center;overflow:hidden}
.shot-editor-modal{background:#0d0d1a;border:1px solid #333;border-radius:12px;width:700px;max-width:90vw;max-height:85vh;display:flex;flex-direction:column}
.shot-editor-modal .sem-header{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:#111;border-bottom:1px solid #333;flex-shrink:0}
...
.shot-editor-modal .sem-body{flex:1;overflow-y:auto;overflow-x:hidden;min-height:0}
.shot-editor-modal .sem-body .CodeMirror{height:60vh;background:#0a0a1a;font-size:12px;line-height:1.5}
...
.dl-clone{background:none;border:none;color:#4fc3f7;cursor:pointer;font-size:11px;padding:0 3px;margin-left:6px;opacity:0.4;font-style:normal;vertical-align:middle;line-height:1}
.dl-clone:hover{opacity:1}
.dl-delete{background:none;border:none;color:#e94560;cursor:pointer;font-size:13px;padding:0 3px;margin-left:2px;opacity:0.5;font-style:normal;vertical-align:middle;line-height:1}
...
<div class="stats">
<div class="stat"><div class="stat-val">${totalSceneCount}</div><div class="stat-lbl">Scenes</div></div>
<div class="stat"><div class="stat-val">${totalShotsAll}</div><div class="stat-lbl">Shots</div></div>
<div class="stat"><div class="stat-val">${(totalShotsAll / Math.max(totalSceneCount, 1)).toFixed(1)}</div><div class="stat-lbl">Avg Shots/Scene</div></div>
...
<div class="nav"><a href="#" onclick="if(window.parent!==window){window.parent.location.hash='#/episode/${EPISODE_ID}'}else{window.location='/'};return false" style="color:#888;text-decoration:none;margin-right:10px;font-size:12px" title="Back to Episode">◀ Back</a><span style="color:#4fc3f7;font-weight:bold;margin-right:4px">${EPISODE_ID}</span>${EPISODE_TITLE ? `<span style="color:#888;font-size:12px;margin-right:12px">${EPISODE_TITLE}</span>` : ''}<label>Jump to: </label><select onchange="document.getElementById('sc-'+this.value)?.scrollIntoView({behavior:'smooth'})">
<option value="">--</option>${pageScenes.map(s => `<option value="${s.sceneNumber}">Scene ${s.sceneNumber} (${s.startTimestamp || '?'}) ${(s.location || '').slice(0, 30)}</option>`).join('')}
</select>
...
<button class="join-btn" id="join-btn" onclick="joinShots()">⊕ Join Selected</button>
</div>
...
function lockShot(scene, shot) {
  ...
  for (const card of shotCards) {
    ...
    if (adj && adj.dataset.scene === scene && adj.dataset.shot === shot) {
      ...
      if (corrections[key] && (corrections[key].startOffsetMs !== 0 || corrections[key].endOffsetMs !== 0)) {
        fetch('/api/apply-corrections', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            episodeId: '${EPISODE_ID}',
            corrections: [{ scene: scene, shot: shot, startOffsetMs: corrections[key].startOffsetMs, endOffsetMs: corrections[key].endOffsetMs }]
          })
          ...
          if (result.success) {
            ...
            var lb = card.querySelector('.lock-btn');
            if (lb) { lb.textContent = '✓ Saved'; lb.style.color = '#4caf50'; }
            var ub = card.querySelector('.unlock-btn');
            if (ub) ub.remove();
            updateCount();
          }
        }).catch(function(e) { console.error('Lock-in save failed:', e.message); });
      }
      break;
    }
  }
...
function updateCount() {
  // No longer tracking corrections count — lock-in saves immediately
}
...
function filterShots() {
  ...
  if (!query) {
    document.querySelectorAll('.shot, .scene, .split-divider').forEach(el => el.classList.remove('search-hidden'));
    countEl.textContent = '';
    return;
  }

  // Hide all split-scene dividers during search
  document.querySelectorAll('.split-divider').forEach(el => el.classList.add('search-hidden'));
...
// --- Split Scene ---
function splitScene(sceneNum, afterShotNum) {
  if (!confirm('Split Scene ' + sceneNum + ' after Shot ' + afterShotNum + '?\\n\\nThis creates two scenes from the current one.')) return;

  // Always fetch fresh for write operations
  fetch('/api/source-json/' + EPISODE_ID).then(function(r) { return r.json(); }).then(function(data) { return JSON.parse(data.json); })
  .then(function(scenes) {
    var sceneIdx = scenes.findIndex(function(s) { return s.sceneNumber === sceneNum; });
    if (sceneIdx === -1) { alert('Scene not found'); return; }
    var scene = scenes[sceneIdx];
    var shots = scene.shots || [];

    // Find the split point
    var splitIdx = shots.findIndex(function(s) { return s.shotNumber === afterShotNum; });
    if (splitIdx === -1 || splitIdx >= shots.length - 1) { alert('Invalid split point'); return; }

    var shotsA = shots.slice(0, splitIdx + 1);
    var shotsB = shots.slice(splitIdx + 1);
    ...
    var splitTimestamp = shotsA[shotsA.length - 1].endTimestamp;
    scene.endTimestamp = splitTimestamp;
    scene.shots = shotsA;

    // Create new scene (second half)
    var newScene = JSON.parse(JSON.stringify(scene)); // deep clone metadata
    newScene.startTimestamp = shotsB[0].startTimestamp;
    newScene.endTimestamp = shots[shots.length - 1].endTimestamp;
    newScene.shots = shotsB;
    newScene.location = scene.location + ' (split)';

    // Renumber shots within each scene
    scene.shots.forEach(function(s, i) { s.shotNumber = i + 1; });
    newScene.shots.forEach(function(s, i) { s.shotNumber = i + 1; });

    // Reassign scene-level dialogue by timestamp
    var allDialogue = (scene.dialogue || []).concat(newScene.dialogue || []);
    var splitSec = pTs(splitTimestamp);
    scene.dialogue = allDialogue.filter(function(d) { return pTs(d.start || d.end || '0:0') < splitSec; });
    newScene.dialogue = allDialogue.filter(function(d) { return pTs(d.start || d.end || '0:0') >= splitSec; });

    // Insert new scene after original
    scenes.splice(sceneIdx + 1, 0, newScene);

    // Renumber all scenes sequentially
    scenes.forEach(function(s, i) { s.sceneNumber = i + 1; });

    window._scenesCache = null; // Don't cache after writes
    return fetch('/api/save-json/${EPISODE_ID}', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ json: JSON.stringify(scenes, null, 2) })
  ...
  .then(function(result) {
    if (result.success) {
      window._scenesCache = null;
      setTimeout(function() { location.reload(); }, 500);
    } else {
      alert('Split failed: ' + (result.error || 'unknown'));
    }
  })
  .catch(function(e) { alert('Split error: ' + e.message); });
}

// --- Re-analyze Scene (Tier 1) ---
function reanalyzeScene(sceneNum) {
  // Show config form before running
  var overlay = document.createElement('div');
  ...
  var modal = document.createElement('div');
  modal.style.cssText = 'background:#1a1a2e;border-radius:12px;padding:24px;width:450px;max-width:90vw;border:1px solid #333';

  modal.innerHTML = '<h3 style="color:#4fc3f7;margin:0 0 16px">\\u{1F504} Tier 1: Re-analyze Scene ' + sceneNum + '</h3>' +
    '<p style="color:#888;font-size:12px;margin-bottom:16px">This will extract the scene\\'s video segment, send it to Gemini for fresh scene detection, and replace this scene with the resulting sub-scenes.</p>' +
    '<label style="color:#aaa;font-size:12px;display:block;margin-bottom:4px">SRT Override (leave empty for auto-detect)</label>' +
    '<div style="display:flex;gap:6px;margin-bottom:12px">' +
    '<input id="ra-srt" type="text" placeholder="e.g. C:\\\\Movies\\\\SW-en-full.srt" style="flex:1;padding:6px 8px;background:#0a0a1a;border:1px solid #444;color:#eee;border-radius:4px;font-size:12px;box-sizing:border-box" />' +
    '<button id="ra-browse" style="background:#333;color:#4fc3f7;border:1px solid #444;padding:6px 12px;border-radius:4px;cursor:pointer;font-size:12px;white-space:nowrap">\\u{1F4C2} Browse</button>' +
    '</div>' +
    '<div id="ra-file-browser" style="display:none;max-height:200px;overflow-y:auto;background:#0a0a1a;border:1px solid #333;border-radius:4px;margin-bottom:12px;font-size:12px"></div>' +
    '<label style="color:#aaa;font-size:12px;display:block;margin-bottom:4px">Region</label>' +
    '<select id="ra-region" style="width:100%;padding:6px 8px;background:#0a0a1a;border:1px solid #444;color:#eee;border-radius:4px;font-size:12px;margin-bottom:16px">' +
    ...
    '<button id="ra-cancel" style="background:#333;color:#eee;border:none;padding:8px 16px;border-radius:4px;cursor:pointer">Cancel</button>' +
    '<button id="ra-go" style="background:#4fc3f7;color:#000;border:none;padding:8px 16px;border-radius:4px;cursor:pointer;font-weight:bold">\\u{1F680} Re-analyze</button>' +
    '</div>';
  ...
  // SRT file browser
  var browserEl = modal.querySelector('#ra-file-browser');
  var srtInput = modal.querySelector('#ra-srt');
  ...
  function loadDir(dir) {
    ...
    fetch(url).then(function(r) { return r.json(); }).then(function(data) {
      ...
      pathRow.style.cssText = 'padding:6px 8px;color:#888;font-size:10px;border-bottom:1px solid #222;word-break:break-all';
      pathRow.textContent = data.path;
      browserEl.appendChild(pathRow);
      // Parent directory
      if (data.parent) {
        var parentRow = document.createElement('div');
        parentRow.style.cssText = 'padding:4px 8px;cursor:pointer;color:#4fc3f7;border-bottom:1px solid #111';
        parentRow.textContent = '\\u{1F4C1} ..';
        parentRow.onclick = function() { loadDir(data.parent); };
        browserEl.appendChild(parentRow);
      }
      // Entries
      (data.entries || []).forEach(function(entry) {
        var row = document.createElement('div');
        row.style.cssText = 'padding:4px 8px;cursor:pointer;border-bottom:1px solid #111;color:' + (entry.type === 'directory' ? '#4fc3f7' : entry.ext === '.srt' ? '#4caf50' : '#888');
        var icon = entry.type === 'directory' ? '\\u{1F4C1} ' : (entry.ext === '.srt' ? '\\u{1F4DD} ' : '\\u{1F4C4} ');
        var size = entry.size ? ' (' + (entry.size > 1e6 ? (entry.size / 1e6).toFixed(1) + ' MB' : (entry.size / 1e3).toFixed(0) + ' KB') + ')' : '';
      ...
      browserEl.style.display = 'block';
      browserEl.innerHTML = '<div style="padding:8px;color:#e94560">Error: ' + e.message + '</div>';
    });
  }

  modal.querySelector('#ra-browse').onclick = function() {
  ...
  modal.querySelector('#ra-cancel').onclick = function() { overlay.remove(); };
  modal.querySelector('#ra-go').onclick = function() {
    var srtPath = srtInput.value.trim();
    var region = modal.querySelector('#ra-region').value;
    // Warn if SRT filename doesn't match episode
    if (srtPath) {
      var srtName = srtPath.replace(/.*[\\/\\\\]/, '').toLowerCase();
      var epLower = EPISODE_ID.toLowerCase();
      if (!srtName.includes(epLower) && !confirm('The SRT file "' + srtName + '" doesn\\'t appear to match episode ' + EPISODE_ID + '.\\n\\nUsing the wrong SRT will corrupt dialogue. Continue anyway?')) {
        return;
      }
    }
    overlay.remove();
    var body = {
      episodeId: '${EPISODE_ID}',
      reanalyzeScene: sceneNum,
      region: region
    };
    if (srtPath) body.srtPath = srtPath;
    startTier2(body, 'Tier 1: Re-analyze Scene ' + sceneNum);
  };
}

// --- Regenerate Scene Frames ---
function regenSceneFrames(sceneNum) {
  var btn = event.target;
  btn.textContent = '⏳ Extracting...';
  btn.disabled = true;
  fetch('/api/regen-frames/' + EPISODE_ID + '/' + sceneNum, { method: 'POST' })
    .then(function(r) { return r.json(); })
    .then(function(result) {
      if (result.success) {
        btn.textContent = '✅ ' + result.extracted + ' frames';
        // Cache-bust all images in this scene
        var sceneDiv = document.querySelectorAll('.scene')[Array.from(document.querySelectorAll('.scene h2')).findIndex(function(h) { return h.textContent.includes('Scene ' + sceneNum); })];
        if (sceneDiv) {
          var imgs = sceneDiv.querySelectorAll('img');
          var bust = Date.now();
          for (var i = 0; i < imgs.length; i++) {
            imgs[i].src = imgs[i].src.split('?')[0] + '?v=' + bust;
          }
        }
        setTimeout(function() { btn.textContent = '🎬 Regen Frames'; btn.disabled = false; }, 2000);
      } else {
        btn.textContent = '❌ ' + (result.error || 'Failed');
        setTimeout(function() { btn.textContent = '🎬 Regen Frames'; btn.disabled = false; }, 3000);
      }
    })
    .catch(function(e) {
...
function editSceneJson(sceneNum) {
  ...
  fetch('/api/source-json/${EPISODE_ID}')
    ...
    .then(function(data) {
      ...
      // Block scroll from reaching the page behind
...
function editShotJson(sceneNum, shotNum) {
  ...
  // Build shot JSON from the page's embedded data (no API fetch needed)
  // The shot card has data attributes we can read, but for full JSON
  // we need to fetch just once. Cache it for subsequent edits.
  var loadShot = function(callback) {
    if (window._scenesCache) {
      callback(window._scenesCache);
      return;
    }
    fetch('/api/source-json/' + EPISODE_ID)
      .then(function(r) { return r.json(); })
      .then(function(data) {
        window._scenesCache = JSON.parse(data.json);
        callback(window._scenesCache);
      })
      .catch(function(e) { alert('Failed to load: ' + e.message); });
  };

  loadShot(function(scenes) {
      var scene = scenes.find(function(s) { return s.sceneNumber === sceneNum; });
      if (!scene) { alert('Scene ' + sceneNum + ' not found'); return; }
...
function saveShotJson(sceneNum, shotNum, textarea, errorDiv, saveBtn) {
  ...
  // Use cached scenes or fetch
  var fetchScenes = window._scenesCache
    ? Promise.resolve({ json: JSON.stringify(window._scenesCache) })
    : fetch('/api/source-json/' + EPISODE_ID).then(function(r) { return r.json(); });
  ...
  fetchScenes
    ...
    .then(function(result) {
      if (result.success) {
        window._scenesCache = null; // Invalidate cache
...
function analyzeTier2(sceneNum, shotNum) {
  // Show config form
  var label = shotNum ? 'Scene ' + sceneNum + ' Shot ' + shotNum : (sceneNum ? 'Scene ' + sceneNum : 'All Shots');
  var overlay = document.createElement('div');
  overlay.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.8);z-index:200;display:flex;align-items:center;justify-content:center';
  overlay.onclick = function(e) { if (e.target === overlay) overlay.remove(); };
  ...
  modal.innerHTML = '<h3 style="color:#ff9800;margin:0 0 16px">\\u26A1 Tier 2: Analyze ' + label + '</h3>' +
    '<p style="color:#888;font-size:12px;margin-bottom:16px">Enriches shot metadata: subject, action, camera, expressions, speaker attribution via audio + multi-frame analysis.</p>' +
    '<label style="color:#aaa;font-size:12px;display:block;margin-bottom:4px">SRT Override (leave empty for auto-detect)</label>' +
    '<div style="display:flex;gap:6px;margin-bottom:12px">' +
    '<input id="t2-srt" type="text" placeholder="e.g. C:\\\\Movies\\\\SW-en-full.srt" style="flex:1;padding:6px 8px;background:#0a0a1a;border:1px solid #444;color:#eee;border-radius:4px;font-size:12px" />' +
    '<button id="t2-srt-browse" style="background:#333;color:#eee;border:none;padding:6px 12px;border-radius:4px;cursor:pointer;font-size:12px;white-space:nowrap">Browse</button>' +
    '</div>' +
    '<label style="color:#aaa;font-size:12px;display:block;margin-bottom:4px">Region</label>' +
    '<select id="t2-region" style="width:100%;padding:6px 8px;background:#0a0a1a;border:1px solid #444;color:#eee;border-radius:4px;font-size:12px;margin-bottom:16px">' +
    '<option value="us-east1">US East (South Carolina)</option>' +
    '<option value="us-central1">US Central (Iowa)</option>' +
    '<option value="us-west1">US West (Oregon)</option>' +
    '<option value="europe-west1">Europe West (Belgium)</option>' +
    '</select>' +
    '<div style="display:flex;gap:8px;justify-content:flex-end">' +
    '<button id="t2-cancel" style="background:#333;color:#eee;border:none;padding:8px 16px;border-radius:4px;cursor:pointer">Cancel</button>' +
    '<button id="t2-go" style="background:#ff9800;color:#000;border:none;padding:8px 16px;border-radius:4px;cursor:pointer;font-weight:bold">\\u26A1 Analyze</button>' +
    '</div>';
  ...
  // SRT Browse button
  modal.querySelector('#t2-srt-browse').onclick = function() {
    openSrtBrowser(modal.querySelector('#t2-srt'));
  };

  modal.querySelector('#t2-cancel').onclick = function() { overlay.remove(); };
  modal.querySelector('#t2-go').onclick = function() {
    var srtPath = modal.querySelector('#t2-srt').value.trim();
    var region = modal.querySelector('#t2-region').value;
    // Warn if SRT filename doesn't seem to match this episode
    if (srtPath) {
      var srtName = srtPath.replace(/.*[\\/\\\\]/, '').toLowerCase();
      var epLower = EPISODE_ID.toLowerCase();
      if (!srtName.includes(epLower) && !confirm('The SRT file "' + srtName + '" doesn\\'t appear to match episode ' + EPISODE_ID + '.\\n\\nUsing the wrong SRT will corrupt dialogue. Continue anyway?')) {
    ...
    overlay.remove();
    var body = { episodeId: '${EPISODE_ID}', region: region };
    if (sceneNum && shotNum) body.shot = sceneNum + '.' + shotNum;
    else if (sceneNum) body.scene = sceneNum;
    if (srtPath) body.srtPath = srtPath;
    startTier2(body, 'Tier 2: ' + label);
  };
}

function analyzeTier2All() {
  analyzeTier2(); // no scene/shot = all
}

// Shared SRT file browser
// --- Generic field editors (floating popup, no reflow) ---
var activeFieldEditor = null;
var activeFieldSave = null;
function closeFieldEditor(doSave) {
  if (activeFieldEditor) {
    if (doSave && activeFieldSave) activeFieldSave();
    activeFieldEditor.remove();
    activeFieldEditor = null;
    activeFieldSave = null;
  }
}
document.addEventListener('mousedown', function(e) {
  if (activeFieldEditor && !activeFieldEditor.contains(e.target)) closeFieldEditor(true);
});

function editSceneField(el, sceneNum, fieldName) {
  closeFieldEditor();
  var isArray = fieldName === 'tags' || fieldName === 'supercutPotential';
  var currentVal = el.textContent.trim();

  var popup = document.createElement('div');
  popup.style.cssText = 'position:fixed;background:#1a1a2e;border:1px solid #4fc3f7;border-radius:6px;padding:8px;z-index:100;box-shadow:0 4px 16px rgba(0,0,0,0.6);min-width:350px;max-width:500px';
  var rect = el.getBoundingClientRect();
  popup.style.left = Math.min(rect.left, window.innerWidth - 520) + 'px';
  popup.style.top = (rect.bottom + 4) + 'px';

  var label = document.createElement('div');
  label.style.cssText = 'color:#888;font-size:10px;margin-bottom:4px';
  label.textContent = 'Scene ' + sceneNum + ' — ' + fieldName + (isArray ? ' (comma-separated)' : '');
  popup.appendChild(label);

  var isLong = fieldName === 'plotSignificance' || fieldName === 'action';
  var input;
  if (isLong) {
    input = document.createElement('textarea');
    input.style.cssText = 'width:100%;padding:6px 8px;background:#0a0a1a;border:1px solid #444;color:#eee;border-radius:3px;font-size:12px;box-sizing:border-box;min-height:80px;resize:vertical';
  } else {
    input = document.createElement('input');
    input.type = 'text';
    input.style.cssText = 'width:100%;padding:6px 8px;background:#0a0a1a;border:1px solid #444;color:#eee;border-radius:3px;font-size:12px;box-sizing:border-box';
  }
  ...
  hint.textContent = (isLong ? 'Ctrl+Enter or click outside' : 'Enter or click outside') + ' to save · Escape to cancel';
  ...
  function save() {
    ...
    fetch('/api/source-json/' + EPISODE_ID).then(function(r) { return r.json(); }).then(function(data) {
      var scenes = JSON.parse(data.json);
      var scene = scenes.find(function(s) { return s.sceneNumber === sceneNum; });
      if (!scene) return;
      scene[fieldName] = isArray ? newVal.split(',').map(function(s) { return s.trim(); }).filter(Boolean) : newVal;
      return fetch('/api/save-json/' + EPISODE_ID, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ json: JSON.stringify(scenes, null, 2) })
      });
    }).catch(function(e) { console.error('Save error:', e.message); });
  }

  input.onkeydown = function(e) {
    if (e.key === 'Enter' && (!isLong || e.ctrlKey)) { e.preventDefault(); save(); }
    if (e.key === 'Escape') { e.preventDefault(); closeFieldEditor(false); }
    e.stopPropagation();
  };

  document.body.appendChild(popup);
  activeFieldEditor = popup;
  activeFieldSave = save;
  setTimeout(function() { input.focus(); input.select(); }, 30);
}

function editShotField(el, sceneNum, shotNum, fieldName) {
  closeFieldEditor();
  var isArray = fieldName === 'tags' || fieldName === 'supercutPotential';
  var isExpr = fieldName === 'characterExpressions';
  var currentVal = el.textContent.trim();
  ...
  var label = document.createElement('div');
  label.style.cssText = 'color:#888;font-size:10px;margin-bottom:4px';
  var helpText = isArray ? ' (comma-separated)' : isExpr ? ' (Name: expression, Name: expression)' : '';
  label.textContent = 'Scene ' + sceneNum + ' Shot ' + shotNum + ' — ' + fieldName + helpText;
  popup.appendChild(label);

  var isLong = fieldName === 'action';
  var input;
  if (isLong) {
    input = document.createElement('textarea');
    input.style.cssText = 'width:100%;padding:6px 8px;background:#0a0a1a;border:1px solid #444;color:#eee;border-radius:3px;font-size:12px;box-sizing:border-box;min-height:80px;resize:vertical';
  ...
  function save() {
    ...
    fetch('/api/source-json/' + EPISODE_ID).then(function(r) { return r.json(); }).then(function(data) {
      ...
      if (isArray) {
        shot[fieldName] = newVal.split(',').map(function(s) { return s.trim(); }).filter(Boolean);
      } else if (isExpr) {
        var obj = {};
        newVal.split(/[·,]/).forEach(function(pair) {
          var ci = pair.indexOf(':');
          if (ci > 0) obj[pair.slice(0, ci).trim()] = pair.slice(ci + 1).trim();
        });
        shot.characterExpressions = obj;
      } else {
        shot[fieldName] = newVal;
      }
...
function openSrtBrowser(inputEl) {
  var currentDir = inputEl.value ? inputEl.value.replace(/[^\\/]+$/, '') : (EPISODE_FILE ? EPISODE_FILE.replace(/[^\\/]+$/, '') : 'C:\\\\');
  var existingPicker = inputEl.parentNode.querySelector('.srt-picker');
  ...
  function loadDir(dir) {
    fetch('/api/browse?dir=' + encodeURIComponent(dir))
      .then(function(r) { return r.json(); })
      .then(function(data) {
        var existingP = inputEl.parentNode.querySelector('.srt-picker');
        if (existingP) existingP.remove();

        var picker = document.createElement('div');
        picker.className = 'srt-picker';
        picker.style.cssText = 'background:#0a0a1a;border:1px solid #444;border-radius:4px;max-height:250px;overflow-y:auto;margin-top:4px;position:absolute;z-index:300;width:100%';

        // Current path header
        var hdr = document.createElement('div');
        hdr.style.cssText = 'padding:4px 8px;font-size:10px;color:#888;border-bottom:1px solid #333;position:sticky;top:0;background:#0a0a1a';
        hdr.textContent = data.path || dir;
        picker.appendChild(hdr);

        // Parent directory
        if (data.parent) {
          var up = document.createElement('div');
          up.style.cssText = 'padding:4px 8px;cursor:pointer;font-size:11px;color:#4fc3f7';
          up.textContent = '⬆ ..';
          up.onclick = function() { loadDir(data.parent); };
          up.onmouseover = function() { this.style.background = '#222'; };
          up.onmouseout = function() { this.style.background = 'none'; };
          picker.appendChild(up);
        }

        (data.entries || []).forEach(function(entry) {
          var row = document.createElement('div');
          row.style.cssText = 'padding:4px 8px;cursor:pointer;font-size:11px;color:#eee';
          if (entry.type === 'directory') {
            row.textContent = '📁 ' + entry.name;
            row.style.color = '#4fc3f7';
            row.onclick = function() { loadDir(entry.path); };
          } else if (entry.ext === '.srt') {
            row.textContent = '📝 ' + entry.name;
            row.style.color = '#a5d6a7';
            row.onclick = function() { inputEl.value = entry.path; picker.remove(); };
          } else {
            row.textContent = '  ' + entry.name;
            row.style.color = '#666';
          }
          row.onmouseover = function() { this.style.background = '#222'; };
          row.onmouseout = function() { this.style.background = 'none'; };
          picker.appendChild(row);
        });

        inputEl.parentNode.style.position = 'relative';
        inputEl.parentNode.appendChild(picker);
...
// --- Edit Scene Characters Inline ---
// Floating character editor popup (same pattern as speaker dropdown — no reflow)
var activeCharEditor = null;
function closeCharEditor() {
  if (activeCharEditor) { activeCharEditor.remove(); activeCharEditor = null; }
}
document.addEventListener('mousedown', function(e) {
  if (activeCharEditor && !activeCharEditor.contains(e.target) && !e.target.classList.contains('chr')) {
    closeCharEditor();
  }
});

function editCharacters(el) {
  closeCharEditor();

  var episodeId = el.dataset.episode;
  var sceneNumber = parseInt(el.dataset.scene);
  var sceneDiv = el.closest('.scene');
  var currentChars = [];
  try { currentChars = JSON.parse(sceneDiv.dataset.characters || '[]'); } catch(e) {}

  // Pre-fetch scenes cache
  if (!window._scenesCache) {
    fetch('/api/source-json/' + episodeId).then(function(r) { return r.json(); }).then(function(data) {
      window._scenesCache = JSON.parse(data.json);
    });
  }

  // Create floating popup (position:fixed, no document reflow)
  var popup = document.createElement('div');
  popup.style.cssText = 'position:fixed;background:#1a1a2e;border:1px solid #4fc3f7;border-radius:6px;padding:8px;z-index:100;box-shadow:0 4px 16px rgba(0,0,0,0.6);min-width:300px';
  var rect = el.getBoundingClientRect();
  ...
  var label = document.createElement('div');
  label.style.cssText = 'color:#888;font-size:10px;margin-bottom:4px';
  label.textContent = 'Scene ' + sceneNumber + ' characters (comma-separated)';
  popup.appendChild(label);
  ...
  input.style.cssText = 'width:100%;padding:6px 8px;background:#0a0a1a;border:1px solid #444;color:#eee;border-radius:3px;font-size:12px;box-sizing:border-box';
  input.placeholder = 'Character 1, Character 2, ...';
  popup.appendChild(input);
  ...
  function save() {
    var newChars = input.value.split(',').map(function(s) { return s.trim(); }).filter(function(s) { return s.length > 0; });
    closeCharEditor();
    if (newChars.length === 0 || JSON.stringify(newChars) === JSON.stringify(currentChars)) return;

    el.textContent = newChars.join(', ');
    sceneDiv.dataset.characters = JSON.stringify(newChars);
    ...
    fetch('/api/source-json/' + episodeId).then(function(r) { return r.json(); }).then(function(data) {
      ...
      if (!result.success) console.error('Failed to save characters:', result.error);
    }).catch(function(e) { console.error('Character save error:', e.message); });
  }

  input.onkeydown = function(e) {
    if (e.key === 'Enter') { e.preventDefault(); save(); }
    if (e.key === 'Escape') { e.preventDefault(); closeCharEditor(); }
    e.stopPropagation();
  };

  document.body.appendChild(popup);
  activeCharEditor = popup;
  setTimeout(function() { input.focus(); input.select(); }, 30);
}

// --- Edit Dialogue Text Inline ---
// Floating dialogue text editor (no reflow)
var activeDlEditor = null;
function closeDlEditor() {
  if (activeDlEditor) { activeDlEditor.remove(); activeDlEditor = null; }
}
document.addEventListener('mousedown', function(e) {
  if (activeDlEditor && !activeDlEditor.contains(e.target) && !e.target.classList.contains('dl-text')) {
    closeDlEditor();
  }
});
...
function editDialogueText(el, sceneNumber, shotNum, dlIdx) {
  ...
  // Pre-fetch scenes cache
  if (!window._scenesCache && episodeId) {
    fetch('/api/source-json/' + encodeURIComponent(episodeId)).then(function(r) { return r.json(); }).then(function(data) {
      window._scenesCache = JSON.parse(data.json);
    });
  }

  var popup = document.createElement('div');
  popup.style.cssText = 'position:fixed;background:#1a1a2e;border:1px solid #4fc3f7;border-radius:6px;padding:8px;z-index:100;box-shadow:0 4px 16px rgba(0,0,0,0.6);width:400px';
  var rect = el.getBoundingClientRect();
  ...
  input.style.cssText = 'width:100%;padding:6px 8px;background:#0a0a1a;border:1px solid #444;color:#eee;border-radius:3px;font-size:12px;font-style:italic;box-sizing:border-box';
  popup.appendChild(input);
  ...
  input.onkeydown = function(e) {
    if (e.key === 'Enter') { e.preventDefault(); save(); }
    if (e.key === 'Escape') { e.preventDefault(); closeDlEditor(); }
    e.stopPropagation();
  };

  document.body.appendChild(popup);
  activeDlEditor = popup;
  setTimeout(function() { input.focus(); input.select(); }, 30);
}
...
// --- Clone Dialogue Line ---
function cloneDialogueLine(btn, sceneNumber, shotNum, dlIdx) {
  var dlContainer = btn.closest('.dialogue-lines');
  var episodeId = dlContainer.dataset.episode;

  // Always fetch fresh — never use stale cache for write operations
  fetch('/api/source-json/' + episodeId).then(function(r) { return r.json(); }).then(function(data) { return JSON.parse(data.json); })
  .then(function(scenes) {
    var scene = scenes.find(function(s) { return s.sceneNumber === parseInt(sceneNumber); });
    if (!scene) throw new Error('Scene not found');
    var shot = (scene.shots || []).find(function(s) { return s.shotNumber === shotNum; });
    ...
    // Deep clone the line and insert after the original
    var clone = JSON.parse(JSON.stringify(shot.dialogue[dlIdx]));
    shot.dialogue.splice(dlIdx + 1, 0, clone);
    window._scenesCache = null; // Don't cache after writes
  ...
  .then(function(result) {
    if (result.success) {
      ...
      alert('Clone failed: ' + (result.error || 'unknown'));
    }
  })
...
return finalHtml;
} // end generateReportHtml

// CLI entrypoint — runs when executed directly
const isMain = process.argv[1] &&
  fileURLToPath(import.meta.url) === path.resolve(process.argv[1]);

if (isMain) {
  const episodeId = process.argv[2] || 'S02E01';
  const videoPath = process.argv[3] || 'C:\\Star Trek\\Star Trek Tng S02e01 The Child (1080P X265 Joy).mp4';

  // Generate full unpaginated report for static file
  const reportHtml = await generateReportHtml(episodeId, videoPath, {
    scenesPerPage: 99999  // all scenes on one page for static file
  });

  const outputDir = path.join(__dirname, 'gemini-analysis', episodeId);
  fs.writeFileSync(path.join(outputDir, 'scene-review-report.html'), reportHtml);

  // Save analysis settings as separate JSON for the hub
  const allScenes = JSON.parse(fs.readFileSync(path.join(outputDir, 'scenes.json'), 'utf-8'));
  const _cpCount = fs.existsSync(path.join(outputDir, 'cut-points.json'))
    ? JSON.parse(fs.readFileSync(path.join(outputDir, 'cut-points.json'), 'utf-8')).length
    : 0;

  const ffmpegDir = path.join(__dirname, 'ffmpeg');
  const subdirs = fs.readdirSync(ffmpegDir).filter(d => d.startsWith('ffmpeg-') && fs.statSync(path.join(ffmpegDir, d)).isDirectory());
  const FFPROBE = path.join(ffmpegDir, subdirs[0], 'bin', 'ffprobe.exe');
  const duration = parseFloat(execSync(`"${FFPROBE}" -v error -show_entries format=duration -of csv=p=0 "${videoPath}"`, { encoding: 'utf-8' }).trim());
  ...
  const settingsData = {
    ...
    episodeId: episodeId,
    localFile: path.basename(videoPath),
  ...
  fs.writeFileSync(path.join(outputDir, 'analysis-settings.json'), JSON.stringify(settingsData, null, 2));

  console.log('✅ Scene Review Report saved');

  if (!process.env.VSTACK_NO_OPEN) {
    try { execSync(`start "" "http://localhost:3333/${episodeId}/scene-review-report.html"`); } catch {}
  }
}
