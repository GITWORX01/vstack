...
.json-editor-area textarea{width:100%;height:100%;background:#0a0a1a;color:#e0e0e0;font-family:'Consolas','Monaco',monospace;font-size:12px;line-height:1.5;border:none;padding:16px;resize:none;outline:none;tab-size:2}
</style></head><body>
...
<div class="nav"><span style="color:#4fc3f7;font-weight:bold;margin-right:12px">${EPISODE_ID}</span><label>Jump to: </label><select onchange="document.getElementById('sc-'+this.value)?.scrollIntoView({behavior:'smooth'})">
...
document.addEventListener('keydown', (e) => {
  if (document.activeElement === document.getElementById('search-input') || document.activeElement === document.getElementById('json-editor-textarea')) return;
...
function openJsonEditor() {
  ...
  textarea.addEventListener('input', () => {
    ...
    jsonValidateTimer = setTimeout(() => jsonEditorValidate(), 500);
    jsonEditorUpdateStats();
  });

  // Escape key handler
  overlay.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { e.preventDefault(); jsonEditorCancel(); }
  });
  ...
  fetch('/api/source-json/${EPISODE_ID}')
    ...
    .catch(err => {
      textarea.value = '// Error loading JSON: ' + err.message;
    });
}
...
function jsonEditorValidate() {
  ...
  try {
    ...
    if (lineMatch) {
      ...
      const upToPos = textarea.value.substring(0, pos);
      const lineNum = upToPos.split('\\n').length;
      msg = '\\u2717 Invalid: ' + msg + ' (line ' + lineNum + ')';
    } else {
      msg = '\\u2717 Invalid: ' + msg;
    }
...
function jsonEditorCancel() {
  const textarea = document.getElementById('json-editor-textarea');
  if (textarea && textarea.value !== jsonEditorOriginal) {
