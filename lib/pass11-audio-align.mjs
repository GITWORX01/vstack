...
function buildShotList(scenes) {
  ...
  for (const scene of scenes) {
    for (const shot of (scene.shots || [])) {
      // Include synthetic shots — they cover real time ranges with dialogue
...
function findShotForTimestamp(shots, timeSec) {
  // Strict matching: word belongs to the shot it falls within
  for (let i = 0; i < shots.length; i++) {
    if (timeSec >= shots[i].startSec && timeSec < shots[i].endSec) {
      return shots[i];
    }
  }
  // In a gap between shots — assign to the NEXT shot (dialogue in gaps
  // perceptually belongs to what comes after, not what came before)
  for (let i = 0; i < shots.length; i++) {
    if (timeSec < shots[i].startSec && (i === 0 || timeSec >= shots[i - 1].endSec)) {
      const gapToNext = shots[i].startSec - timeSec;
      if (gapToNext < 1.0) return shots[i]; // within 1s gap → next shot
    }
  }
...
for (const line of matchedLines) {
  ...
  if (line.words.length >= 2) {
    ...
    const BOUNDARY_SNAP = 0.15; // words within 150ms of shot end snap to next shot
    const wordShots = line.words.map((w, wi) => ({
      word: w.word,
      start: w.start,
      end: w.end,
      shot: findShotForTimestamp(shotList, w.start),
    }));
    // Snap boundary words: if a word is within 150ms of its shot's end AND the next word
    // is in a different shot, push this word to the next shot (it perceptually belongs there)
    for (let wi = 0; wi < wordShots.length - 1; wi++) {
      const ws = wordShots[wi];
      const next = wordShots[wi + 1];
      if (ws.shot && next.shot && ws.shot !== next.shot) {
        const distToEnd = ws.shot.endSec - ws.start;
        if (distToEnd < BOUNDARY_SNAP) {
          ws.shot = next.shot; // snap to next shot
        }
      }
    }
    ...
    // Multi-segment (cross-boundary split) — align SRT words to Whisper words
    // Find the split point in SRT text by matching Whisper boundary words
    const allWhisperWords = segments.flatMap(s => s.words);

    // Build SRT-to-Whisper word alignment via greedy forward match
    const srtNorm = srtTextWords.map(w => normalize(w));
    const whNorm = allWhisperWords.map(w => normalize(w.word));
    // For each segment, find how many SRT words correspond to its Whisper words
    const segSrtRanges = [];
    let srtIdx = 0;
    for (let si = 0; si < segments.length; si++) {
      const seg = segments[si];
      const segWhisperCount = seg.words.length;
      const segStartSrtIdx = srtIdx;

      // Match this segment's Whisper words to SRT words
      let whIdx = segments.slice(0, si).reduce((s, g) => s + g.words.length, 0);
      let matched = 0;
      while (matched < segWhisperCount && srtIdx < srtTextWords.length) {
        const srtW = srtNorm[srtIdx];
        const whW = whNorm[whIdx + matched];
        // Check if SRT word matches Whisper word (fuzzy: shared prefix >= 3 chars)
        if (srtW === whW || (srtW.length >= 3 && whW && whW.startsWith(srtW.slice(0, 3))) ||
            (whW && whW.length >= 3 && srtW.startsWith(whW.slice(0, 3)))) {
          matched++;
          srtIdx++;
        } else {
          // SRT word doesn't match any Whisper word — it's an extra SRT word, keep it in this segment
          srtIdx++;
        }
      }
      // If last segment, take all remaining SRT words
      if (si === segments.length - 1) srtIdx = srtTextWords.length;
      segSrtRanges.push({ start: segStartSrtIdx, end: srtIdx });
    }

    for (let si = 0; si < segments.length; si++) {
      const seg = segments[si];
      if (!seg.shot) continue;
      const range = segSrtRanges[si];
      const segSrtWords = srtTextWords.slice(range.start, range.end);
