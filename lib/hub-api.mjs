...
export function handleApiRequest(url, req, res) {
  ...
  try {
    ...
    // POST /api/reanalyze-shot — reanalyze a single shot (split into finer shots)
    if (pathname === '/api/reanalyze-shot' && req.method === 'POST') {
      ...
      req.on('end', async () => {
        try {
          ...
          const shotIdx = scene.shots.findIndex(s => s.shotNumber === parseInt(shotNumber));
          ...
          console.log(`[reanalyze-shot] ${episodeId} Scene ${sceneNumber} Shot ${shotNumber}: ${shot.startTimestamp}-${shot.endTimestamp} (${duration.toFixed(1)}s)`);
          ...
          // Run Gemini Pass B on just this time range
          const prompt = `Analyze this video from ${fmtTs(startSec)} to ${fmtTs(endSec)}.

For every camera cut or visual change in this range, provide a shot object with:
shotNumber, startTimestamp (MM:SS.s), endTimestamp (MM:SS.s), shotType, subject, action, characterExpressions (object with single string values), cameraMovement, tags (array), supercutPotential (array)
...
- Output ONLY a JSON array [ ... ]`;
          ...
          const url = `https://${REGION}-aiplatform.googleapis.com/v1/projects/${PROJECT}/locations/${REGION}/publishers/google/models/gemini-2.5-pro:generateContent`;

          // Use async exec so the event loop stays free (browser won't timeout)
          const result = await new Promise((resolve, reject) => {
            exec(
              `curl -s --max-time 300 "${url}" -H "Authorization: Bearer ${token}" -H "Content-Type: application/json" -d @"${tmpReq}"`,
          ...
          if (response.error) {
            res.writeHead(500);
            res.end(JSON.stringify({ error: 'Gemini error: ' + (response.error.message || '').slice(0, 100) }));
            return;
          }
          ...
          if (!jsonMatch) {
            res.writeHead(500);
            res.end(JSON.stringify({ error: 'No valid JSON in Gemini response' }));
            return;
          }

          const newShots = JSON.parse(jsonMatch[0]);
          console.log(`[reanalyze-shot] Got ${newShots.length} new shots for the range`);

          if (newShots.length <= 1) {
            res.writeHead(200);
            res.end(JSON.stringify({ success: true, newShotCount: newShots.length, message: 'Gemini returned the same single shot — no further splits possible.' }));
            return;
          }

          // Replace the single shot with the new shots
          scene.shots.splice(shotIdx, 1, ...newShots);

          // Renumber all shots in the scene
          ...
          const FFMPEG = path.join(ffmpegDir2, ffmpegSubdirs2[0], 'bin', 'ffmpeg.exe');
          const framesDir = path.join(ANALYSIS_DIR, episodeId, 'frames');

          // Clear old frames for this scene (renumbered)
          const oldFrames = fs.readdirSync(framesDir).filter(f => f.startsWith(`sc${sceneNumber}_`));
          ...
          for (const sh of scene.shots) {
            ...
            sh._frameFirst = `sc${sceneNumber}_sh${sh.shotNumber}_first.jpg`;
            sh._frameLast = `sc${sceneNumber}_sh${sh.shotNumber}_last.jpg`;
            try {
              execSync(`"${FFMPEG}" -ss ${ss.toFixed(3)} -i "${videoPath}" -vframes 1 -q:v 3 -vf "scale=320:-1" "${path.join(framesDir, sh._frameFirst)}" -y`, { stdio: 'pipe', timeout: 10000 });
              execSync(`"${FFMPEG}" -ss ${es.toFixed(3)} -i "${videoPath}" -vframes 1 -q:v 3 -vf "scale=320:-1" "${path.join(framesDir, sh._frameLast)}" -y`, { stdio: 'pipe', timeout: 10000 });
              extracted++;
            } catch {}
          }

          // Rebuild DB
          try { rebuildEpisode(episodeId, scenesPath); } catch {}

          // Rebuild report
          try {
            const rebuildScript = path.join(__dirname, 'rebuild-report.mjs');
            execSync(`node "${rebuildScript}" "${episodeId}" "${videoPath}"`, { cwd: __dirname, stdio: 'pipe', timeout: 60000 });
          } catch {}

          res.writeHead(200);
          res.end(JSON.stringify({
            success: true,
            oldShot: { scene: sceneNumber, shot: shotNumber, duration: duration.toFixed(1) + 's' },
            newShotCount: newShots.length,
            framesExtracted: extracted,
            message: `Split into ${newShots.length} shots (was ${duration.toFixed(0)}s). Report rebuilt.`
          }));
        } catch (e) {
          res.writeHead(500);
          res.end(JSON.stringify({ error: 'Reanalyze failed: ' + e.message }));
        }
      });
      return true;
    }
