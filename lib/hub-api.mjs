  ...
  try {
    ...
    if (exportEpMatch && req.method === 'GET') {
      ...
      try {
        ...
        // Copy scenes.json + cut-points.json
        fs.copyFileSync(scenesPath, path.join(tmpDir, episodeId, 'scenes.json'));
        const cutPointsPath = path.join(epDir, 'cut-points.json');
        if (fs.existsSync(cutPointsPath)) {
          fs.copyFileSync(cutPointsPath, path.join(tmpDir, episodeId, 'cut-points.json'));
        }
    ...
    if (importEpMatch && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          // Copy scenes.json
          fs.copyFileSync(scenesSource, existingScenes);

          // Copy cut-points.json if present
          const cpSource = path.join(path.dirname(scenesSource), 'cut-points.json');
          if (fs.existsSync(cpSource)) {
            fs.copyFileSync(cpSource, path.join(epDir, 'cut-points.json'));
          }
    ...
    if (exportMatch && req.method === 'GET') {
      ...
      try {
        ...
        // Copy frames + cut-points per episode
        for (const ep of episodes) {
          ...
          // Cut points
          const cpPath = path.join(epSrcDir, 'cut-points.json');
          if (fs.existsSync(cpPath)) {
            fs.copyFileSync(cpPath, path.join(destEpDir, 'cut-points.json'));
          }
      ...
      req.on('end', () => {
        try {
          ...
          for (const ep of manifest.episodes) {
            ...
            // Copy cut-points.json if present
            const cpSrc = path.join(tmpDir, ep.id, 'cut-points.json');
            if (fs.existsSync(cpSrc)) {
              fs.copyFileSync(cpSrc, path.join(epDir, 'cut-points.json'));
            }
