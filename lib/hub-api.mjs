    ...
    if (saveJsonMatch && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          // Clear frames and re-extract (scene numbers may have changed)
          ...
          res.end(JSON.stringify({
            ...
            framesExtracted,
            message: `Saved ${scenes.length} scenes, ${totalShots} shots. DB rebuilt. ${framesExtracted} frames extracted. Report rebuilt.`
          }));
        } catch (e) {
          res.writeHead(500);
          res.end(JSON.stringify({ error: e.message }));
