...
export function handleApiRequest(url, req, res) {
  ...
  try {
    ...
    if (pathname === '/api/tier2/start' && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          const { episodeId, scene, shot, region } = JSON.parse(body);
          if (!episodeId) {
            res.writeHead(400);
            res.end(JSON.stringify({ error: 'episodeId required' }));
            return;
          ...
          if (fs.existsSync(statusFile)) {
            try {
              ...
              if (s.phase === 'running') {
                ...
                if (age < 120) {
                  res.writeHead(409);
                  res.end(JSON.stringify({ error: 'Tier 2 already running', status: s }));
                  return;
                }
              }
            } catch {}
          ...
          const cmd = `node "${tier2Script}" ${args.map(a => `"${a}"`).join(' ')}`;
