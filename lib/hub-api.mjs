...
export function handleApiRequest(url, req, res) {
  ...
  try {
    ...
    // POST /api/regen-frames/:episodeId/:sceneNum — re-extract frames for one scene
    const regenFramesMatch = pathname.match(/^\/api\/regen-frames\/([^/]+)\/(\d+)$/);
    if (regenFramesMatch && req.method === 'POST') {
      const episodeId = decodeURIComponent(regenFramesMatch[1]);
      const sceneNum = parseInt(regenFramesMatch[2]);
      const videoPath = findVideoForEpisode(episodeId);
      if (!videoPath) {
        res.writeHead(404);
      ...
      if (!fs.existsSync(scenesPath)) {
        res.writeHead(404);
        res.end(JSON.stringify({ error: 'No scenes.json for ' + episodeId }));
        return true;
      }
      try {
        ...
        if (!scene) {
          res.writeHead(404);
          res.end(JSON.stringify({ error: 'Scene ' + sceneNum + ' not found' }));
          return true;
        }
        ...
        for (const sh of (scene.shots || [])) {
          const ss = Math.max(0, parseTs(sh.startTimestamp) + 0.7);
          const es = Math.max(0, parseTs(sh.endTimestamp) - 0.7);
          const firstPath = path.join(framesDir, sh._frameFirst || `sc${sceneNum}_sh${sh.shotNumber}_first.jpg`);
          const lastPath = path.join(framesDir, sh._frameLast || `sc${sceneNum}_sh${sh.shotNumber}_last.jpg`);
          try {
            execSync(`"${FFMPEG}" -ss ${ss.toFixed(3)} -i "${videoPath}" -vframes 1 -q:v 3 -vf scale=320:-1 "${firstPath}" -y`, { stdio: 'pipe', timeout: 10000 });
            execSync(`"${FFMPEG}" -ss ${es.toFixed(3)} -i "${videoPath}" -vframes 1 -q:v 3 -vf scale=320:-1 "${lastPath}" -y`, { stdio: 'pipe', timeout: 10000 });
            extracted++;
          } catch {}
        }
        res.writeHead(200);
        res.end(JSON.stringify({ success: true, extracted, total: (scene.shots || []).length }));
      } catch (e) {
        res.writeHead(500);
        res.end(JSON.stringify({ error: e.message }));
      }
    ...
    if (saveJsonMatch && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          // Build index of existing frame files by their old _frameFirst/_frameLast reference
          // Map: old filename -> full path (for any old shot that had a frame file)
          const existingFramesByName = {};
          try {
            const allFrameFiles = fs.readdirSync(framesDir).filter(f => f.endsWith('.jpg'));
            for (const f of allFrameFiles) {
              existingFramesByName[f] = path.join(framesDir, f);
            }
          } catch {}

          // Also build timestamp -> old file path map from old scenes data
          const existingFramesByTs = {};
          for (const sc of oldScenes) {
            for (const sh of (sc.shots || [])) {
              if (sh._frameFirst && existingFramesByName[sh._frameFirst]) {
                existingFramesByTs[sh.startTimestamp + '_first'] = existingFramesByName[sh._frameFirst];
              }
              if (sh._frameLast && existingFramesByName[sh._frameLast]) {
                existingFramesByTs[sh.endTimestamp + '_last'] = existingFramesByName[sh._frameLast];
              }
            }
          }
          ...
          // Check if scene numbering changed (split/reanalyze) — if so, we need to remap all frames
          const numberingChanged = scenes.some((sc, i) => {
            const old = oldScenes[i];
            if (!old) return true;
            if (sc.sceneNumber !== old.sceneNumber) return true;
            if ((sc.shots?.length || 0) !== (old.shots?.length || 0)) return true;
            return false;
          }) || scenes.length !== oldScenes.length;
          ...
          for (const sc of scenes) {
            for (const sh of (sc.shots || [])) {
              ...
              // If numbering didn't change and files exist, skip
              if (!numberingChanged && fs.existsSync(firstPath) && fs.existsSync(lastPath)) continue;

              // Always use timestamp-based lookup when numbering changed
              const tsFirst = sh.startTimestamp + '_first';
              const tsLast = sh.endTimestamp + '_last';
              let firstOk = false;
              let lastOk = false;

              // Copy from the correct old file by timestamp
              if (existingFramesByTs[tsFirst]) {
                try { fs.copyFileSync(existingFramesByTs[tsFirst], firstPath); firstOk = true; framesCopied++; } catch {}
              } else if (fs.existsSync(firstPath)) {
                firstOk = true; // file exists and no better source
              }
              if (existingFramesByTs[tsLast]) {
                try { fs.copyFileSync(existingFramesByTs[tsLast], lastPath); lastOk = true; framesCopied++; } catch {}
              } else if (fs.existsSync(lastPath)) {
                lastOk = true;
              }
