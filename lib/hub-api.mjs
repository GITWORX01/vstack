    ...
    if (pathname === '/api/start-analysis' && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          const analyzeScript = path.join(__dirname, 'analyze-episode.mjs');
          const cmd = 'node "' + analyzeScript + '" "' + episodeId + '" "' + videoPath + '" --skip-upload' + (region ? ' --region ' + region : '');
          const child = exec(cmd, {
            cwd: __dirname,
            env: { ...process.env, VSTACK_NO_OPEN: '1', MEDIA_DIR: path.dirname(videoPath) }
          });
          child.unref();
          res.writeHead(202);
          res.end(JSON.stringify({
