    ...
    if (pathname === '/api/reanalyze-shot' && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          // Run the actual work in background via child process
          const mediaDir = process.env.MEDIA_DIR || 'C:\\Star Trek';
          ...
          const script = `
            ...
            const ANALYSIS_DIR = ${JSON.stringify(ANALYSIS_DIR)};
            const episodeId = ${JSON.stringify(episodeId)};
            const sceneNum = ${parseInt(sceneNumber)};
            const shotNum = ${parseInt(shotNumber)};
            const mediaDir = ${JSON.stringify(mediaDir)};
            const GCLOUD_PATH = ${JSON.stringify(GCLOUD_PATH)};
            const statusFile = path.join(ANALYSIS_DIR, episodeId, '_reanalyze-status.json');

            function parseTs(ts) { if (!ts) return 0; const [m,s]=ts.split(':'); return parseInt(m)*60+parseFloat(s); }
            function fmtTs(sec) { const m=Math.floor(sec/60),s=sec%60; return String(m).padStart(2,'0')+':'+s.toFixed(1).padStart(4,'0'); }
            function writeStatus(obj) { fs.writeFileSync(statusFile, JSON.stringify(obj)); }
            ...
            try {
              ...
              let srtSection = '';
              const srtFiles = fs.readdirSync(mediaDir).filter(f => f.toLowerCase().endsWith('.srt'));
              const srtMatch = srtFiles.find(f => new RegExp('s0?' + epMatch[1] + 'e0?' + epMatch[2], 'i').test(f));
              if (srtMatch) {
                const srtContent = fs.readFileSync(path.join(mediaDir, srtMatch), 'utf-8');
                ...
                for (const block of blocks) {
                  ...
                  if (text && st >= startSec - 2 && st <= endSec + 2) subs.push({ st, en, text });
                }
                if (subs.length > 0) {
                  srtSection = '\\n\\nDIALOGUE (from subtitles — identify who speaks each line):\\n' + subs.map(s => '[' + fmtTs(s.st) + '] ' + s.text).join('\\n');
                }
              }

              // Build rich prompt with scene context
              const prompt = 'Analyze this video from ' + fmtTs(startSec) + ' to ' + fmtTs(endSec) + '.\\n' +
                ...
                'For every camera cut or visual change, provide a shot object with:\\n' +
                '- shotNumber, startTimestamp (MM:SS.s), endTimestamp (MM:SS.s)\\n' +
                ...
                '- dialogue (array of {speaker, text, start, end} — use EXACT subtitle text below, identify speaker by watching who talks)\\n' +
                '- tags (array of searchable keywords — be generous)\\n' +
                '- supercutPotential (array of compilation categories)\\n' +
                srtSection + '\\n\\n' +
                'RULES: Cover EVERY second from ' + fmtTs(startSec) + ' to ' + fmtTs(endSec) + '. Continuous timestamps. Sub-second precision. Output ONLY a JSON array [ ... ]';
              ...
              const reqBody = JSON.stringify({
                ...
                generationConfig:{temperature:0.1,maxOutputTokens:16384,mediaResolution:'MEDIA_RESOLUTION_LOW'}
              });

              const tmpReq = path.join(ANALYSIS_DIR, episodeId, '_reanalyze_req.json');
              fs.writeFileSync(tmpReq, reqBody);

              const url = 'https://us-east1-aiplatform.googleapis.com/v1/projects/data-mind-456822-q3/locations/us-east1/publishers/google/models/gemini-2.5-pro:generateContent';
              const result = execSync('curl -s --max-time 300 "' + url + '" -H "Authorization: Bearer ' + token + '" -H "Content-Type: application/json" -d @"' + tmpReq + '"', {encoding:'utf-8',maxBuffer:10*1024*1024});
              if (fs.existsSync(tmpReq)) fs.unlinkSync(tmpReq);

              const response = JSON.parse(result);
              if (response.error) { writeStatus({status:'error',error:response.error.message}); process.exit(1); }

              let text = (response.candidates?.[0]?.content?.parts?.[0]?.text||'').replace(/\`\`\`json\\s*/g,'').replace(/\`\`\`\\s*/g,'').replace(/,\\s*}/g,'}').replace(/,\\s*]/g,']');
              const m = text.match(/\\[[\\s\\S]*\\]/);
              if (!m) { writeStatus({status:'error',error:'No JSON in response'}); process.exit(1); }

              const newShots = JSON.parse(m[0]);
              if (newShots.length <= 1) { writeStatus({status:'done',newShotCount:1,message:'Single shot — no splits possible'}); process.exit(0); }
              ...
              const ffmpegDir = path.join(${JSON.stringify(__dirname)}, 'ffmpeg');
              ...
              const old = fs.readdirSync(framesDir).filter(f => f.startsWith('sc' + sceneNum + '_'));
              for (const f of old) fs.unlinkSync(path.join(framesDir, f));

              let extracted = 0;
              for (const sh of scene.shots) {
                sh._frameFirst = 'sc'+sceneNum+'_sh'+sh.shotNumber+'_first.jpg';
                sh._frameLast = 'sc'+sceneNum+'_sh'+sh.shotNumber+'_last.jpg';
                try {
                  execSync('"'+FFMPEG+'" -ss '+parseTs(sh.startTimestamp).toFixed(3)+' -i "'+videoPath+'" -vframes 1 -q:v 3 -vf scale=320:-1 "'+path.join(framesDir,sh._frameFirst)+'" -y',{stdio:'pipe',timeout:10000});
                  execSync('"'+FFMPEG+'" -ss '+parseTs(sh.endTimestamp).toFixed(3)+' -i "'+videoPath+'" -vframes 1 -q:v 3 -vf scale=320:-1 "'+path.join(framesDir,sh._frameLast)+'" -y',{stdio:'pipe',timeout:10000});
                  extracted++;
                } catch{}
              }
              fs.writeFileSync(scenesPath, JSON.stringify(scenes, null, 2));
              ...
              try { execSync('node "' + path.join(${JSON.stringify(__dirname)}, 'rebuild-report.mjs') + '" "' + episodeId + '" "' + videoPath + '"', {cwd:${JSON.stringify(__dirname)},stdio:'pipe',timeout:60000}); } catch{}

              writeStatus({ status: 'done', newShotCount: newShots.length, framesExtracted: extracted, message: 'Split into ' + newShots.length + ' shots. Report rebuilt.' });
            } catch(e) {
              writeStatus({ status: 'error', error: e.message });
            }
          `;

          // Spawn background process
          const child = exec(`node -e "${script.replace(/"/g, '\\"').replace(/\n/g, ' ')}"`, {
