...
export function handleApiRequest(url, req, res) {
  ...
  try {
    ...
    if (saveJsonMatch && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          // Smart video file lookup (works for all episode ID formats)
            ...
            if (ffSubs.length > 0) {
              ...
              for (const sc of scenes) {
                for (const sh of (sc.shots || [])) {
                  ...
                  // Check if timestamps changed or frames missing
          ...
          // Static report rebuild skipped — report is now served dynamically via /api/report-html/
    ...
    if (pathname === '/api/tier2/start' && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          const { episodeId, scene, shot, region, reanalyzeScene } = JSON.parse(body);
          if (!episodeId) {
            res.writeHead(400);
            res.end(JSON.stringify({ error: 'episodeId required' }));
            return;
          ...
          if (reanalyzeScene) {
            ...
            cmd = `node "${reanalyzeScript}" "${episodeId}" "${reanalyzeScene}" --region ${region || 'us-east1'}`;
          } else {
