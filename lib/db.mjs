...
export function rebuildEpisode(episodeId, scenesJsonPath) {
  const db = getDb();
  ...
  const rebuild = db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO episodes (id, scene_count, shot_count, analyzed_at, json_hash)
      VALUES (?, ?, ?, datetime('now'), ?)
    ...
    `).run(episodeId, scenes.length, totalShots, newHash);
...
if (args.includes('--rebuild')) {
  ...
  console.log('Usage: node db.mjs [--rebuild [EPISODE]] [--search "query"] [--search-dialogue "query"] [--verify] [--stats]');
}
