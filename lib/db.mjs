...
CREATE TABLE IF NOT EXISTS episodes (
  ...
  is_copy INTEGER DEFAULT 0,             -- 1 = cloned copy (excluded from supercut search)
  cloned_from TEXT,                      -- Original episode ID this was cloned from
...
export function getDb() {
  if (!_db) {
    ...
    try { _db.exec('ALTER TABLE episodes ADD COLUMN is_copy INTEGER DEFAULT 0'); } catch {}
    try { _db.exec('ALTER TABLE episodes ADD COLUMN cloned_from TEXT'); } catch {}
  }
  return _db;
}
...
export function search(query, options = {}) {
  ...
  let sql = `
    ...
    AND (e.is_copy = 0 OR e.is_copy IS NULL)
  `;
  const params = [query];
...
export function createSeries(id, title, contentContext) {
  const db = getDb();
  db.prepare('INSERT OR REPLACE INTO series (id, title, content_context) VALUES (?, ?, ?)').run(id, title, contentContext || '');
  return { id, title, contentContext };
}
