...
async function analyzeScenes(gcsUri, shots, srtEntries, durationSec) {
  ...
  const prompt = `Analyze this video (which includes audio) and break it into SCENES. A scene is a continuous segment in one location with the same group of characters/subjects.
${contextLine}
...
- Total video duration is approximately ${fmtTs(durationSec)}`;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      const result = execSync(
        `curl -s --max-time 1800 "${url}" -H "Authorization: Bearer ${token}" -H "Content-Type: application/json" -d @"${curlReqPath}"`,
        { encoding: 'utf-8', maxBuffer: 100 * 1024 * 1024, timeout: 1860000 }
      );
