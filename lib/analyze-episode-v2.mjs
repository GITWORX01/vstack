...
- Total video duration is approximately ${fmtTs(durationSec)}`;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      const result = execSync(
        `curl -s --max-time 5400 "${url}" -H "Authorization: Bearer ${token}" -H "Content-Type: application/json" -d @"${curlReqPath}"`,
        { encoding: 'utf-8', maxBuffer: 100 * 1024 * 1024, timeout: 5460000 }
      );
