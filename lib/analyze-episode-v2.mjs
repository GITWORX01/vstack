...
const CHUNK_THRESHOLD_MIN = 40;  // Split videos longer than this into chunks
const CHUNK_OVERLAP_SEC = 5;     // Overlap between chunks for scene continuity
...
function loadSRT() {
  ...
  for (const block of blocks) {
    ...
    // Split multi-character lines: "- Line one - Line two"
    const dashSplit = text.match(/^-\s+(.+?)\s+-\s+(.+)$/);
    if (dashSplit) {
      const halfDur = (end - start) / 2;
      entries.push({ start, end: start + halfDur, text: dashSplit[1].trim() });
      entries.push({ start: start + halfDur, end, text: dashSplit[2].trim() });
      continue;
    }

    // Split "text SPEAKER: text" mid-line speaker changes
    const midSpeaker = text.match(/^(.+?)\s+([A-Z][A-Z\s]*?):\s+(.+)$/);
    if (midSpeaker && midSpeaker[1].length > 3) {
      const halfDur = (end - start) / 2;
      entries.push({ start, end: start + halfDur, text: midSpeaker[1].replace(/^-\s*/, '').trim() });
      entries.push({ start: start + halfDur, end, text: midSpeaker[3].trim() });
      continue;
    }

    // Strip leading speaker tags: "SPEAKER: text" → just "text"
    const speakerTag = text.match(/^([A-Z][A-Z\s]+?):\s*(.+)/);
    if (speakerTag) {
      entries.push({ start, end, text: speakerTag[2].trim() });
      continue;
    }
...
// ── Step 5: Gemini scene analysis ───────────────────────────────────

// ── Tier 1: Scene analysis — chunked for long videos ────────────────
// For videos ≤ 40 min: single Gemini call (original behavior)
// For videos > 40 min: split into ~40 min chunks, analyze each, merge
// Returns scene-level metadata. Shot boundaries come from PySceneDetect.

function buildScenePrompt(chunkDurationSec, chunkOffsetSec) {
  ...
  // When analyzing a chunk, tell Gemini the real-world time offset
  const offsetNote = chunkOffsetSec > 0
    ? `\nIMPORTANT: This video clip starts at ${fmtTs(chunkOffsetSec)} in the full movie. All timestamps you return must be RELATIVE TO THIS CLIP (starting from 00:00.0), NOT the full movie. I will adjust them afterward.\n`
    : '';
...
- Total video duration is approximately ${fmtTs(chunkDurationSec)}`;
}

async function callGeminiForScenes(gcsUri, prompt, chunkLabel) {
  ...
  const tmpReq = path.join(OUT_DIR, `_req_scenes_${chunkLabel}.json`);
  fs.writeFileSync(tmpReq, body);

  const totalAttempts = MAX_RETRIES + 1;
  for (let attempt = 1; attempt <= totalAttempts; attempt++) {
    console.log(`   🔄 [${chunkLabel}] Attempt ${attempt}/${totalAttempts}...`);
    writeStatus({ phase: 'analyzing', chunk: chunkLabel, attempt, maxRetries: totalAttempts });
    ...
    try {
      ...
      if (response.error) {
        ...
        console.log(`   ⚠️  [${chunkLabel}] API error: ${msg}`);
      ...
      const cost = logCost(`tier1-${EPISODE_ID}-${chunkLabel}`, usage);
      console.log(`   ✅ [${chunkLabel}] Scene analysis complete — $${cost.toFixed(4)}`);
      ...
      if (finishReason === 'MAX_TOKENS') {
        console.log(`   ⚠️  [${chunkLabel}] Response truncated — some scenes may be missing`);
      }

      // Join all response parts (thinking tokens may be in separate parts)
      ...
      if (!match) {
        console.log(`   ⚠️  [${chunkLabel}] No JSON array found in response`);
        if (attempt < totalAttempts) { await sleep(15000); continue; }
        return null;
      }

      const scenes = JSON.parse(match[0]);
      console.log(`   📊 [${chunkLabel}] ${scenes.length} scenes detected`);

      // Save raw response for debugging
      fs.writeFileSync(path.join(OUT_DIR, `tier1-raw-response-${chunkLabel}.json`), JSON.stringify(response, null, 2));

      if (fs.existsSync(tmpReq)) fs.unlinkSync(tmpReq);
      return scenes;

    } catch (err) {
      console.log(`   ⚠️  [${chunkLabel}] Failed: ${err.message?.slice(0, 120)}`);
...
function addTimestampOffset(ts, offsetSec) {
  const sec = parseTs(ts) + offsetSec;
  return fmtTs(Math.max(0, sec));
}

async function analyzeScenes(gcsUri, shots, srtEntries, durationSec, lowResPath) {
  const chunkThresholdSec = CHUNK_THRESHOLD_MIN * 60;

  if (durationSec <= chunkThresholdSec) {
    // ── Short video: single call (original behavior) ──
    ...
    const prompt = buildScenePrompt(durationSec, 0);
    const scenes = await callGeminiForScenes(gcsUri, prompt, 'full');
    if (scenes) {
      fs.writeFileSync(path.join(OUT_DIR, 'tier1-raw-response.json'),
  ...
  const numChunks = Math.ceil(durationSec / chunkThresholdSec);
  const chunkDuration = Math.ceil(durationSec / numChunks);

  console.log(`\n🤖 Step 5: Gemini scene analysis — CHUNKED (Tier 1)`);
  console.log(`   Model: ${MODEL} | Region: ${REGION}`);
  console.log(`   Duration: ${fmtTs(durationSec)} → ${numChunks} chunks of ~${(chunkDuration / 60).toFixed(0)} min each`);
  console.log(`   Shots detected locally: ${shots.length} (for reference only)`);
  writeStatus({ phase: 'analyzing', step: 'Tier 1 — chunked analysis', totalChunks: numChunks, totalShots: shots.length });
  ...
  for (let ci = 0; ci < numChunks; ci++) {
    ...
    const chunkEnd = Math.min((ci + 1) * chunkDuration + CHUNK_OVERLAP_SEC, durationSec);
    const chunkLen = chunkEnd - chunkStart;
    const chunkLabel = `chunk${ci + 1}`;

    console.log(`\n   ── Chunk ${ci + 1}/${numChunks}: ${fmtTs(chunkStart)} → ${fmtTs(chunkEnd)} (${(chunkLen / 60).toFixed(1)} min) ──`);
    writeStatus({ phase: 'analyzing', chunk: ci + 1, totalChunks: numChunks, chunkStart: fmtTs(chunkStart), chunkEnd: fmtTs(chunkEnd) });

    // Split the low-res video into this chunk using ffmpeg
    const chunkFile = path.join(OUT_DIR, `${EPISODE_ID}_chunk${ci + 1}.mp4`);
    console.log(`   🔪 Splitting: ${fmtTs(chunkStart)} → ${fmtTs(chunkEnd)}`);
    try {
      execSync(
        `"${FFMPEG}" -ss ${chunkStart.toFixed(3)} -i "${lowResPath}" -t ${chunkLen.toFixed(3)} -c copy "${chunkFile}" -y`,
        { stdio: 'pipe', timeout: 120000 }
      );
      console.log(`   ✅ Chunk file: ${fmtBytes(getFileSize(chunkFile))}`);
    } catch (e) {
      console.log(`   ⚠️  ffmpeg split failed: ${e.message?.slice(0, 100)}`);
      // Fallback: re-encode (slower but more reliable for some codecs)
      try {
        execSync(
          `"${FFMPEG}" -ss ${chunkStart.toFixed(3)} -i "${lowResPath}" -t ${chunkLen.toFixed(3)} -c:v libx264 -preset fast -crf 28 -c:a aac -b:a 8k -ac 1 "${chunkFile}" -y`,
          { stdio: 'pipe', timeout: 300000 }
        );
        console.log(`   ✅ Chunk file (re-encoded): ${fmtBytes(getFileSize(chunkFile))}`);
      } catch (e2) {
        console.log(`   ❌ Chunk split failed entirely: ${e2.message?.slice(0, 100)}`);
        continue;
      }
    }

    // Upload chunk to GCS
    const chunkGcsUri = `${GCS_BUCKET}/${EPISODE_ID}_chunk${ci + 1}.mp4`;
    console.log(`   📤 Uploading chunk ${ci + 1}...`);
    try {
      const ext = process.platform === 'win32' ? '.cmd' : '';
      execSync(`"${path.join(GCLOUD_PATH, 'gsutil' + ext)}" cp "${chunkFile}" "${chunkGcsUri}"`,
        { stdio: 'pipe', timeout: 600000 });
      console.log(`   ✅ Uploaded: ${chunkGcsUri}`);
    } catch (e) {
    ...
    // Analyze this chunk
    const prompt = buildScenePrompt(chunkLen, chunkStart);
    const chunkScenes = await callGeminiForScenes(chunkGcsUri, prompt, chunkLabel);

    // Clean up chunk file (local) — keep GCS for potential re-runs
    try { fs.unlinkSync(chunkFile); } catch {}

    if (!chunkScenes || chunkScenes.length === 0) {
      console.log(`   ⚠️  Chunk ${ci + 1} returned no scenes — gap in coverage`);
      continue;
    }

    // Adjust timestamps: Gemini returns chunk-relative times, add the chunk offset
    for (const scene of chunkScenes) {
      scene.startTimestamp = addTimestampOffset(scene.startTimestamp, chunkStart);
      scene.endTimestamp = addTimestampOffset(scene.endTimestamp, chunkStart);
      scene.sceneNumber = sceneNumberOffset + scene.sceneNumber;
    }

    // Handle overlap: if this chunk's first scene overlaps with last chunk's last scene,
    // merge them or trim the boundary
    if (ci > 0 && allScenes.length > 0 && chunkScenes.length > 0) {
      const lastPrev = allScenes[allScenes.length - 1];
      const firstCurr = chunkScenes[0];
      const prevEnd = parseTs(lastPrev.endTimestamp);
      const currStart = parseTs(firstCurr.startTimestamp);

      if (currStart < prevEnd) {
        // Overlap detected — trim the previous scene's end to meet the new scene's start
        const midpoint = (prevEnd + currStart) / 2;
        lastPrev.endTimestamp = fmtTs(midpoint);
        firstCurr.startTimestamp = fmtTs(midpoint);
        console.log(`   🔗 Trimmed overlap at ${fmtTs(midpoint)} between chunks ${ci} and ${ci + 1}`);
      } else if (currStart > prevEnd + 1) {
        // Gap detected — extend previous scene's end to meet
        lastPrev.endTimestamp = firstCurr.startTimestamp;
        console.log(`   🔗 Filled gap: extended previous scene to ${firstCurr.startTimestamp}`);
      }
    }

    sceneNumberOffset = allScenes.length + chunkScenes.length;
    allScenes.push(...chunkScenes);
    console.log(`   📊 Running total: ${allScenes.length} scenes`);
  }

  if (allScenes.length === 0) return null;

  // Final renumber
  allScenes.forEach((sc, i) => { sc.sceneNumber = i + 1; });
  console.log(`\n   ✅ Chunked analysis complete: ${allScenes.length} scenes across ${numChunks} chunks`);

  // Save merged raw response
  fs.writeFileSync(path.join(OUT_DIR, 'tier1-raw-response.json'), JSON.stringify(allScenes, null, 2));
...
function mergeData(shots, sceneMetadata, srtEntries) {
  ...
  for (const entry of srtEntries) {
    ...
    // Rule: if dialogue starts in the last 500ms of a shot, assign to next shot
    const BOUNDARY_THRESHOLD = 0.5; // seconds
    ...
    for (let si = 0; si < scene.shots.length; si++) {
      ...
      if (entry.start >= shStart - 0.3 && entry.start < shEnd) {
        // Check if dialogue is in the last 500ms of this shot
        if (shEnd - entry.start < BOUNDARY_THRESHOLD && si + 1 < scene.shots.length) {
          // Assign to next shot instead
...
async function main() {
  ...
  // Step 5: Tier 1 — Scene analysis (single call or chunked for long videos)
  const geminiScenes = await analyzeScenes(gcsUri, shots, srtEntries, durationSec, lowResPath);
  ...
  if (srtEntries.length > 0) {
    ...
    for (const scene of geminiScenes) {
      ...
      // Also assign dialogue to individual shots within the scene (including synthetic)
      for (const shot of (scene.shots || [])) {
