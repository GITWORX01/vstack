...
const GCS_BUCKET = process.env.GCS_BUCKET || 'gs://tng-video-analysis-east';
...
function detectShots() {
  ...
  const lowResPath = path.join(OUT_DIR, `${EPISODE_ID}_${LOWRES_HEIGHT}p_noaudio.mp4`);
...
function createLowRes() {
  ...
  if (fs.existsSync(lowResPath)) {
    ...
    console.log(`\n📐 Step 2: Low-res version exists (${fmtBytes(size)})`);
    return lowResPath;
  }

  console.log(`\n📐 Step 2: Creating ${LOWRES_HEIGHT}p no-audio version`);
  console.log(`   Source: ${fmtBytes(getFileSize(LOCAL_FILE))}`);
  writeStatus({ phase: 'creating-lowres', step: `${LOWRES_HEIGHT}p no-audio` });

  execSync(
    `"${FFMPEG}" -i "${LOCAL_FILE}" -vf "scale=-2:${LOWRES_HEIGHT}" -c:v libx264 -preset fast -crf 28 -an "${lowResPath}" -y`,
...
async function analyzeScenes(gcsUri, shots, srtEntries, durationSec) {
  ...
  // Build SRT section — include for context but tell Gemini NOT to repeat it back
  let srtSection = '';
  if (srtEntries.length > 0) {
    srtSection = '\n\nSUBTITLE CONTEXT (use this to help identify characters, understand dialogue, and detect scene changes — but do NOT include the full dialogue text in your output):\n' +
      srtEntries.map(s => `[${fmtTs(s.start)}] "${s.text}"`).join('\n');
  }

  const prompt = `Analyze this video and break it into SCENES. A scene is a continuous segment in one location with the same group of characters.
...
- characters (array of character names present — use the subtitle context to help identify who is in each scene)
- speakers (array of character names who SPEAK in this scene — identify by watching who talks, using subtitle timing to match)
- mood (emotional tone of the scene)
- plotSignificance (1-2 sentence summary of what happens and why it matters — use subtitle context to understand the dialogue)
...
IMPORTANT: Do NOT include a "dialogue" field. Do NOT repeat the subtitle text back. We will match dialogue to scenes separately using timestamps. Just identify which characters speak (in the "speakers" field).
${srtSection}
  ...
  for (const scene of geminiScenes) {
    ...
    scene.shots = shots.filter(s => s.startSec >= sceneStart - 0.5 && s.endSec <= sceneEnd + 0.5)
      .map(s => ({
        ...
        // Tier 2 will add: shotType, subject, action, characterExpressions, etc.
        _tier: 1, // marker that this shot only has Tier 1 data
      }));
  }

  // Match SRT dialogue to scenes by timestamp
  if (srtEntries.length > 0) {
    console.log(`   💬 Matching ${srtEntries.length} subtitle lines to scenes...`);
    let matched = 0;
    for (const scene of geminiScenes) {
      const sceneStart = parseTs(scene.startTimestamp);
      const sceneEnd = parseTs(scene.endTimestamp);
      scene.dialogue = srtEntries
        ...
        .map(s => ({
          ...
          // Speaker comes from the "speakers" field Gemini identified
          // We can refine this in Tier 2 with per-shot attribution
        }));
      matched += scene.dialogue.length;
    }
    console.log(`   ✅ ${matched} dialogue lines matched to scenes`);
  }
