...
function decompressShot(compressed, srtEntries, sceneSrt) {
  ...
  // Gemini sometimes renumbers lines starting from 1 instead of using our global indices.
  // We try: 1) global index match, 2) scene-relative index (1-based offset into sceneSrt)
  if (compressed.d) {
    result.dialogue = [];
    ...
    for (const entry of entries) {
      ...
      // Try 1: global SRT index
      let srtLine = srtEntries.find(s => s.index === lineNum);

      // Try 2: scene-relative (1-based) — Gemini often renumbers from 1
