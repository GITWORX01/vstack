...
async function analyzeScene(scene, shots, videoPath, srtEntries) {
  ...
  if (sceneSrt.length > 0) {
    srtSection = '\nDIALOGUE (identify who speaks each line):\n' +
      sceneSrt.map(s => `[${s.index}] "${s.text}"`).join('\n');
  }
...
- Output ONLY a JSON array [ ... ]`;
  ...
  for (let attempt = 1; attempt <= totalAttempts; attempt++) {
    ...
    try {
      ...
      return { results: JSON.parse(match[0]), sceneSrt };
...
function decompressShot(compressed, srtEntries) {
  const shotTypeMap = {
  ...
  // Decompress expressions: "Picard:stern,Riker:amused"
  if (compressed.e) {
    result.characterExpressions = {};
    for (const pair of compressed.e.split(',')) {
      const [char, expr] = pair.split(':').map(s => s.trim());
      if (char && expr) result.characterExpressions[char] = expr;
    }
  }

  // Decompress tags: "picard,bridge,command"
  if (compressed.k) {
    result.tags = compressed.k.split(',').map(t => t.trim()).filter(Boolean);
  }

  // Decompress supercut: "Picard Leadership,Command"
  if (compressed.p) {
    result.supercutPotential = compressed.p.split(',').map(t => t.trim()).filter(Boolean);
  }

  // Decompress dialogue: "Picard|47,Data|48"
  if (compressed.d) {
    result.dialogue = [];
    for (const entry of compressed.d.split(',')) {
      const [speaker, lineNumStr] = entry.split('|').map(s => s.trim());
      const lineNum = parseInt(lineNumStr);
      const srtLine = srtEntries.find(s => s.index === lineNum);
      if (srtLine) {
  ...
  for (let i = 0; i < scenesToProcess.length; i++) {
    ...
    if (result) {
      ...
      for (const compressed of results) {
        ...
        const decompressed = decompressShot(compressed, srtEntries);
