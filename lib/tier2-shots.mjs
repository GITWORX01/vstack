...
function loadSrt() {
  ...
  try {
    ...
    // Try any SRT file if there's only one
...
- Output ONLY a JSON array [ ... ]`;
  ...
  for (let attempt = 1; attempt <= totalAttempts; attempt++) {
    ...
    try {
      ...
      const cost = logCost(`tier2-${EPISODE_ID}-scene${sceneNum}-${shots.length}shots`, usage);
...
function decompressShot(compressed, srtEntries, sceneSrt) {
  ...
  if (compressed.d) {
    ...
    for (const entry of entries) {
      ...
      // No global fallback — pulling lines by global index grabs wrong dialogue
      // from the start of the movie. If scene-relative lookup fails, skip it.
    ...
    if (result) {
      ...
      for (const compressed of results) {
        ...
        if (decompressed.dialogue?.length) {
          // Only overwrite dialogue if we got actual text (not [line N] placeholders)
          const hasRealText = decompressed.dialogue.some(d => d.text && !d.text.match(/^\[line \d+\]$/));
          if (hasRealText) {
            // Merge speaker assignments into existing dialogue if possible
            const existing = shot.dialogue || [];
            if (existing.length > 0 && existing.length === decompressed.dialogue.length) {
              // Same count — update speakers from Gemini but keep existing text/timestamps
              for (let di = 0; di < existing.length; di++) {
                if (decompressed.dialogue[di].speaker && decompressed.dialogue[di].speaker !== 'Unknown') {
                  existing[di].speaker = decompressed.dialogue[di].speaker;
                }
                // Only replace text if decompressed has real text
                if (decompressed.dialogue[di].text && !decompressed.dialogue[di].text.match(/^\[line \d+\]$/)) {
                  if (!existing[di].text || existing[di].text.match(/^\[line \d+\]$/)) {
                    existing[di].text = decompressed.dialogue[di].text;
                  }
                }
              }
              shot.dialogue = existing;
            } else {
              shot.dialogue = decompressed.dialogue;
            }
          } else if (shot.dialogue?.length) {
            // Only update speakers from Gemini, keep existing text and timestamps
            for (const dd of decompressed.dialogue) {
              if (!dd.speaker || dd.speaker === 'Unknown') continue;
              const lineMatch = dd.text?.match(/^\[line (\d+)\]$/);
              if (lineMatch) {
                const idx = parseInt(lineMatch[1]);
                // Find matching existing line by index
                if (idx < shot.dialogue.length && shot.dialogue[idx].speaker === 'Unknown') {
                  shot.dialogue[idx].speaker = dd.speaker;
                }
              }
            }
          }
          totalDialogue += (shot.dialogue || []).length;
        }
        shot._tier = 2;
        totalAnalyzed++;
      }
