...
const SRT_OVERRIDE = args.find(a => a === '--srt') ? args[args.indexOf('--srt') + 1] : null;

if (!EPISODE_ID || isNaN(SCENE_NUMBER)) {
  console.error('Usage: node reanalyze-scene.mjs EPISODE_ID SCENE_NUMBER [--region us-east1] [--srt path/to/file.srt]');
  process.exit(1);
}
...
function loadSrt() {
  ...
  // Try 2: Look for SRT next to the video file (from settings.json) — match by video filename
  const settingsPath = path.join(OUT_DIR, 'settings.json');
  if (fs.existsSync(settingsPath)) {
    try {
      const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
      ...
      if (vp) {
        ...
        // Try matching by video filename first
        ...
        // Only use first SRT if there's exactly one (unambiguous)
...
async function callGeminiForScenes(gcsUri, prompt) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      const cost = logCost(`reanalyze-${EPISODE_ID}-scene${SCENE_NUMBER}`, usage);
...
async function main() {
  ...
  // Build map of old frame filenames BEFORE renumbering
  const oldFrameMap = {}; // oldName -> path
  for (const sc of allScenes) {
    for (const sh of (sc.shots || [])) {
      if (sh._frameFirst) oldFrameMap[sh._frameFirst] = path.join(FRAMES_DIR, sh._frameFirst);
      if (sh._frameLast) oldFrameMap[sh._frameLast] = path.join(FRAMES_DIR, sh._frameLast);
    }
  }
  ...
  allScenes.forEach((sc, i) => {
    if (sc.shots) {
      sc.shots.forEach((sh, si) => {
        const oldFirst = sh._frameFirst;
        const oldLast = sh._frameLast;
        sh._frameFirst = `sc${i + 1}_sh${si + 1}_first.jpg`;
        sh._frameLast = `sc${i + 1}_sh${si + 1}_last.jpg`;
        // Track old -> new mapping for rename
        sh._oldFirst = oldFirst;
        sh._oldLast = oldLast;
      });
    }
    sc.sceneNumber = i + 1;
  ...
  // ── 11. Rename existing frames + extract new ones ──────────────────

  console.log(`\nUpdating frame files...`);
  writeStatus({ phase: 'extracting-frames', step: 'Updating frame files' });

  fs.mkdirSync(FRAMES_DIR, { recursive: true });
  let framesRenamed = 0;
  let framesExtracted = 0;

  // First pass: rename/copy existing frames to new names
  for (const sc of allScenes) {
    for (const sh of (sc.shots || [])) {
      const newFirstPath = path.join(FRAMES_DIR, sh._frameFirst);
      const newLastPath = path.join(FRAMES_DIR, sh._frameLast);

      // Skip if new filename already exists
      if (!fs.existsSync(newFirstPath) && sh._oldFirst && oldFrameMap[sh._oldFirst] && fs.existsSync(oldFrameMap[sh._oldFirst])) {
        try { fs.copyFileSync(oldFrameMap[sh._oldFirst], newFirstPath); framesRenamed++; } catch {}
      }
      if (!fs.existsSync(newLastPath) && sh._oldLast && oldFrameMap[sh._oldLast] && fs.existsSync(oldFrameMap[sh._oldLast])) {
        try { fs.copyFileSync(oldFrameMap[sh._oldLast], newLastPath); framesRenamed++; } catch {}
      }
  ...
  console.log(`  ${framesRenamed} frames renamed/copied`);

  // Second pass: extract frames for new sub-scenes (ones that don't have files yet)
  ...
  for (let si = newStartIdx; si < newEndIdx; si++) {
    ...
    for (const shot of (scene.shots || [])) {
      ...
      try {
        if (!fs.existsSync(firstPath)) execSync(`"${FFMPEG}" -ss ${firstSec.toFixed(3)} -i "${videoPath}" -vframes 1 -q:v 3 -vf "scale=320:-1" "${firstPath}" -y`, { stdio: 'pipe', timeout: 10000 });
        if (!fs.existsSync(lastPath)) execSync(`"${FFMPEG}" -ss ${lastSec.toFixed(3)} -i "${videoPath}" -vframes 1 -q:v 3 -vf "scale=320:-1" "${lastPath}" -y`, { stdio: 'pipe', timeout: 10000 });
        framesExtracted++;
      } catch {}
    }
  }
  console.log(`  ${framesExtracted} new frames extracted`);
