...
async function callGemini(prompt, gcsUri, cacheName, label) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      const cost = promptTokens * inputRate / 1e6 + outputTokens * 10 / 1e6 + thinkingTokens * inputRate / 1e6;
...
async function callGeminiWithClip(prompt, clipGcsUri, label) {
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      const outputTokens = usage.candidatesTokenCount || 0;
      const thinkingTokens = usage.thoughtsTokenCount || 0;
      // Clip tokens are NOT cached — billed at full rate
      const cost = promptTokens * 2.00 / 1e6 + outputTokens * 10 / 1e6 + thinkingTokens * 2.00 / 1e6;
      onSuccess();
...
async function main() {
  ...
  if (!SKIP_ANALYSIS && !REPORT_ONLY) {
    ...
    console.log(`\n🤖 Analyzing with ${MODEL} (context cached 💰)...`);
