...
const ALL_REGIONS = ['us-east1', 'us-central1', 'europe-west1', 'asia-northeast1'];
...
function getToken() {
  try {
    const token = execSync(`"${path.join(GCLOUD_PATH, 'gcloud')}" auth print-access-token`,
...
function preflightChecks() {
  ...
  if (!SKIP_UPLOAD && !REPORT_ONLY) {
    try {
      execSync(`"${path.join(GCLOUD_PATH, 'gsutil')}" ls "${GCS_BUCKET}/" > /dev/null 2>&1`, { timeout: 15000 });
    } catch {
  ...
  try {
    const billingResult = execSync(
      `"${path.join(GCLOUD_PATH, 'gcloud')}" billing projects describe ${PROJECT} --format="value(billingEnabled)"`,
...
function estimateCost(durationSec) {
  ...
  const tokensPerChunk = 24897 * CHUNK_MINUTES; // ~25K tokens/min at LOW
  const inputCost = tokensPerChunk * chunks * 2 / 1e6; // $2/M input
  const cachedInputCost = tokensPerChunk * (chunks - 1) * 0.2 / 1e6 + tokensPerChunk * 1 * 2 / 1e6; // first full, rest cached
  const outputCost = chunks * 15 * 250 * 10 / 1e6; // ~15 scenes × 250 tokens × $10/M
  const totalUncached = inputCost + outputCost;
  const totalCached = cachedInputCost + outputCost;
...
async function main() {
  ...
  writeStatus({ phase: 'starting', duration: dur, chunks: numChunks, estimatedCost: cost.totalCached });
