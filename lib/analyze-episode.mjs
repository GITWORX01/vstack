...
const CACHE_TTL_SECONDS = 3600;       // 1 hour cache TTL (plenty for all chunks)
...
async function createVideoCache(gcsUri) {
  ...
  if (fs.existsSync(cacheFile)) {
    ...
    try {
      ...
      const checkResult = execSync(
        `curl -s --max-time 30 "https://${REGION}-aiplatform.googleapis.com/v1/${cached}" -H "Authorization: Bearer ${token}"`,
        { encoding: 'utf-8' }
      );
  ...
  const token = getToken();
  const modelPath = `projects/${PROJECT}/locations/${REGION}/publishers/google/models/${MODEL}`;
  ...
  try {
    const result = execSync(
      `curl -s --max-time 300 "https://${REGION}-aiplatform.googleapis.com/v1/projects/${PROJECT}/locations/${REGION}/cachedContents" ` +
...
async function deleteVideoCache(cacheName) {
  ...
  try {
    const token = getToken();
    execSync(
      `curl -s -X DELETE "https://${REGION}-aiplatform.googleapis.com/v1/${cacheName}" -H "Authorization: Bearer ${token}"`,
      { encoding: 'utf-8' }
    );
    const cacheFile = path.join(OUT_DIR, '_cache-id.txt');
...
async function analyzeChunk(gcsUri, startMin, endMin, srtEntries, cacheName) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      // When using context cache: video is already cached, only send the text prompt.
      // Without cache: send video + prompt together (original behavior).
      ...
      const inputRate = cachedTokens > 0 ? 0.20 : 2.00; // cached = 90% discount ($0.20/M vs $2.00/M)
      ...
      const cacheInfo = cachedTokens > 0 ? ` 💰 ${(cachedTokens/1000).toFixed(0)}K cached` : '';
      console.log(`  ✅ ${scenes.length} scenes, ${totalShots} shots ($${cost.toFixed(3)}${cacheInfo})`);
      return scenes;
