...
if (!GCS_BUCKET || !PROJECT) { console.error('❌ GCS_BUCKET and GCP_PROJECT env vars required'); process.exit(1); }
...
// CACHING REMOVED — was costing $50+/day in storage fees. See analyze-episode-v2.mjs for current pipeline.
...
// ── CACHING REMOVED ──────────────────────────────────────────────────
// Context caching was removed — it cost $50+/day in storage fees.
// Use analyze-episode-v2.mjs instead (240p + single API call, no caching).
// ─────────────────────────────────────────────────────────────────────
...
// CACHING REMOVED — all calls send video directly (no cache)

async function callGemini(prompt, gcsUri, cacheName_UNUSED, label) {
...
async function main() {
  ...
  if (!SKIP_ANALYSIS && !REPORT_ONLY) {
    // No caching — send video directly in each API call

    console.log(`\n🤖 Analyzing — Pass A: ${MODEL_SCENES} (full episode) | Pass B: ${MODEL_SHOTS} (scene clips)...`);

    // ── Pass A: Single call for ALL scenes (full episode, cached) ─────
    // No chunking — Gemini sees the full episode for better scene boundaries
    const scenesFile = path.join(OUT_DIR, 'scenes-passA.json');
    let scenes = [];
    ...
    if (fs.existsSync(scenesFile)) {
      try {
        ...
        if (scenes.length > 0) {
          console.log(`  ⏭️  Pass A cached (${scenes.length} scenes)`);
        } else {
          scenes = [];
        }
      } catch { scenes = []; }
    ...
    if (scenes.length === 0) {
      writeStatus({ phase: 'analyzing', passA: true, step: 'Identifying all scenes' });
      console.log(`  📋 Pass A: Scenes for full episode (0-${durMin.toFixed(0)}min)...`);
      const scenePrompt = buildScenePrompt(0, durMin, srtEntries);
      const sceneResult = await callGemini(scenePrompt, gcsUri, cacheName, `scenes full-episode`);

      if (!sceneResult) {
        writeStatus({ phase: 'failed', error: 'Pass A failed — could not identify scenes' });
        throw new Error('Pass A failed');
      }

      scenes = sceneResult.data;
      // Save Pass A result for resume
      fs.writeFileSync(scenesFile, JSON.stringify(scenes, null, 2));
      console.log(`  ✅ ${scenes.length} scenes ($${sceneResult.cost.toFixed(3)}${sceneResult.cacheInfo})`);
    }

    // ── Pass B: Shots per scene (using scene clips) ─────────────────
    console.log(`  🎬 Pass B: Shots for ${scenes.length} scenes (using scene clips)...`);
    let totalShots = 0;
    let totalShotCost = 0;
    const clipsDir = path.join(OUT_DIR, '_scene_clips');
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      // Skip if scene already has shots (resume support)
      if (scene.shots?.length > 0) {
        totalShots += scene.shots.length;
        process.stdout.write(`    Scene ${i+1}: ${scene.shots.length} shots (cached)  `);
        continue;
      }
      ...
      try {
        ...
        // Save progress after each scene (resume support)
        fs.writeFileSync(scenesFile, JSON.stringify(scenes, null, 2));

      } catch (err) {
        console.log(`    Scene ${i+1}: ❌ clip error: ${err.message?.slice(0, 60)}`);
        scene.shots = [];
      }
    }

    // Clean up GCS clips
    console.log(`\n  🧹 Cleaning up ${clipCleanup.length} scene clips from GCS...`);
    for (const gcsPath of clipCleanup) {
      try {
        execSync(`"${path.join(GCLOUD_PATH, 'gsutil' + CMD_EXT)}" -q rm "${gcsPath}"`, { stdio: 'pipe', timeout: 10000 });
      } catch {}
    ...
    console.log(`  ✅ Total: ${totalShots} shots ($${totalShotCost.toFixed(3)})`);
    ...
    if (dialogueAssigned > 0) console.log(`  💬 ${dialogueAssigned} dialogue lines assigned to shots`);

    allChunks.push(scenes);

    // No cache to clean up
  } else {
  ...
  // Merge (with single full-episode pass, this just renumbers)
  console.log(`\n🔗 Processing ${allChunks.length} chunk(s)...`);
