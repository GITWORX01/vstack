  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (noShots.length > 0 && attempt < MAX_RETRIES) {
        console.log(`  ⚠️  ${noShots.length} scenes without shots, retrying...`);
...
async function main() {
  ...
  if (!SKIP_ANALYSIS && !REPORT_ONLY) {
    ...
    for (let i = 0; i < numChunks; i++) {
      ...
      if (i < numChunks - 1) { console.log(`  ⏳ Cooldown...`); await sleep(CHUNK_COOLDOWN_MS); }
    }
  } else {
