...
export function generateDaVinciXML(projectName) {
  ...
  // Build asset-clip elements with volume and startSecOffset
  // FCPXML adjust-volume uses dB: 20*log10(ratio)
  const CLIP_VOLUME_DB = '-10.5dB';   // 30% → ~-10.5 dB
  const MUSIC_VOLUME_DB = '-12dB';    // 25% → ~-12 dB
  const INTERJ_VOLUME_DB = '0dB';     // Full volume (boosted files already amplified)
  let timelineOffset = 0;
  const assetClipElements = [];
  const subtitleEntries = []; // For SRT export
  ...
  for (let i = 0; i < clips.length; i++) {
    ...
    assetClipElements.push(
      ...
      `                    <adjust-volume amount="${CLIP_VOLUME_DB}"/>\n` +
      `                </asset-clip>`
    );

    // Collect subtitle entries for SRT
    if (clip.dialogueText || clip.action) {
      const text = clip.dialogueText || clip.action || '';
      if (text.length > 0) {
        subtitleEntries.push({
          index: subtitleEntries.length + 1,
          startSec: timelineOffset,
          endSec: timelineOffset + duration,
  ...
  const { execSync: execSyncFn } = childProcess;
  ...
  const FFPROBE = ffmpegSubs.length > 0 ? path.join(ffmpegDir, ffmpegSubs[0], 'bin', 'ffprobe.exe') : 'ffprobe';

  // Build asset elements with actual durations
  const assetElements = Object.values(assets).map(asset => {
    // Get duration via ffprobe
    let durationSec = 7200; // default 2 hours
    try {
      durationSec = parseFloat(execSyncFn(
        `"${FFPROBE}" -v error -show_entries format=duration -of csv=p=0 "${asset.filePath}"`,
        { encoding: 'utf-8', timeout: 15000 }
      ).trim()) || 7200;
    } catch {}
    const assetDuration = toTime(durationSec);

    // Windows file URL: use forward slashes, encode special chars
    const fileUrl = 'file://localhost/' + asset.filePath.replace(/\\/g, '/').replace(/ /g, '%20').replace(/\(/g, '%28').replace(/\)/g, '%29');

    return `        <asset id="${asset.id}" name="${escapeXml(asset.name)}" uid="${asset.id}" ` +
      `start="0s" duration="${assetDuration}" hasVideo="1" format="r1" hasAudio="1" ` +
      `audioChannels="2" audioRate="48000">\n` +
      `            <media-rep kind="original-media" sig="${asset.id}" src="${fileUrl}"/>\n` +
      `        </asset>`;
  });

  // ── Find and add audio assets (music + interjections) ──
  const audioDir = path.resolve(__dirname, '..', 'extracted', 'public', 'audio');
  const musicDir = path.resolve(__dirname, '..', 'extracted', 'public', 'music');
  const audioAssets = [];
  const audioClipElements = [];

  // Music track
  const musicDataPath = path.resolve(__dirname, '..', 'extracted', 'src', 'musicData.json');
  if (fs.existsSync(musicDataPath)) {
    try {
      const musicData = JSON.parse(fs.readFileSync(musicDataPath, 'utf-8'));
      for (const track of musicData) {
        const musicFile = path.resolve(__dirname, '..', 'extracted', 'public', track.file);
        if (!fs.existsSync(musicFile)) continue;

        const musicId = `r${assetId++}`;
        let musicDuration = (track.durationMs || 150000) / 1000;
        const musicUrl = 'file://localhost/' + musicFile.replace(/\\/g, '/').replace(/ /g, '%20');

        audioAssets.push(
          `        <asset id="${musicId}" name="Music — ${escapeXml(track.prompt || 'Background')}" uid="${musicId}" ` +
          `start="0s" duration="${toTime(musicDuration)}" hasVideo="0" format="r1" hasAudio="1" ` +
          `audioChannels="2" audioRate="48000">\n` +
          `            <media-rep kind="original-media" sig="${musicId}" src="${musicUrl}"/>\n` +
          `        </asset>`
        );

        // Add as connected clip on the first spine clip (plays full duration)
        audioClipElements.push(
          `                <asset-clip ref="${musicId}" lane="1" offset="0s" name="Music" ` +
          `start="0s" duration="${toTime(Math.min(musicDuration, timelineOffset))}" audioRole="music">\n` +
          `                    <adjust-volume amount="${MUSIC_VOLUME_DB}"/>\n` +
          `                </asset-clip>`
        );
      }
    } catch {}
  ...
  // Interjection audio files
  const interjDir = path.join(audioDir, 'interjections');
  if (fs.existsSync(interjDir)) {
    const interjFiles = fs.readdirSync(interjDir).filter(f => f.endsWith('.mp3')).sort();
    // Space interjections evenly across the timeline
    const interval = timelineOffset / (Math.min(interjFiles.length, 15) + 1);

    for (let ii = 0; ii < Math.min(interjFiles.length, 15); ii++) {
      const intFile = path.join(interjDir, interjFiles[ii]);
      const intId = `r${assetId++}`;

      let intDuration = 2; // default
      try {
        intDuration = parseFloat(execSyncFn(
          `"${FFPROBE}" -v error -show_entries format=duration -of csv=p=0 "${intFile}"`,
          { encoding: 'utf-8', timeout: 5000 }
        ).trim()) || 2;
      } catch {}

      const intUrl = 'file://localhost/' + intFile.replace(/\\/g, '/').replace(/ /g, '%20');
      const intOffset = (ii + 1) * interval;

      audioAssets.push(
        `        <asset id="${intId}" name="${escapeXml(interjFiles[ii])}" uid="${intId}" ` +
        `start="0s" duration="${toTime(intDuration)}" hasVideo="0" format="r1" hasAudio="1" ` +
        `audioChannels="2" audioRate="48000">\n` +
        `            <media-rep kind="original-media" sig="${intId}" src="${intUrl}"/>\n` +
        `        </asset>`
      );

      // Interjection clips: use audioBoost from config if available, else default
      const intBoost = config.interjectionBoost || 1.0;
      const intVolDb = intBoost === 1.0 ? INTERJ_VOLUME_DB : `${(20 * Math.log10(intBoost)).toFixed(1)}dB`;
      audioClipElements.push(
        `                <asset-clip ref="${intId}" lane="2" offset="${toTime(intOffset)}" name="${escapeXml(interjFiles[ii])}" ` +
        `start="0s" duration="${toTime(intDuration)}" audioRole="effects">\n` +
        `                    <adjust-volume amount="${intVolDb}"/>\n` +
        `                </asset-clip>`
      );
    }
  }

  // Build connected audio elements (attached to first spine clip)
  const connectedAudio = audioClipElements.length > 0
    ? '\n' + audioClipElements.join('\n')
    : '';
        ...
        <event name="${escapeXml(config.query || projectName)}">
            <project name="${escapeXml(projectName)}" uid="${projectName}" modDate="${new Date().toISOString().replace('T', ' ').slice(0, 19)} +0000">
                <sequence format="r1" duration="${totalDuration}" tcStart="0s" tcFormat="NDF" audioLayout="stereo" audioRate="48k">
                    <spine>
${assetClipElements[0]}${connectedAudio}
${assetClipElements.slice(1).join('\n')}
                    </spine>
                </sequence>
            </project>
        </event>
  ...
  if (subtitleEntries.length > 0) {
    const srtLines = subtitleEntries.map(entry => {
      const fmtTime = (sec) => {
      ...
      return `${entry.index}\n${fmtTime(entry.startSec)} --> ${fmtTime(entry.endSec)}\n${entry.text}\n`;
    });
    srtPath = path.join(projectDir, `${projectName}.srt`);
    fs.writeFileSync(srtPath, srtLines.join('\n'), 'utf-8');
    console.log(`  SRT subtitles exported: ${srtPath} (${subtitleEntries.length} entries)`);
  }

  console.log(`  DaVinci Resolve XML exported: ${xmlPath}`);
  console.log(`  ${clips.length} clips, ${timelineOffset.toFixed(1)}s total`);

  return { xmlPath, srtPath, clipCount: clips.length, totalDuration: timelineOffset };
}
