...
function loadSrt() {
  // Search multiple directories for SRT files
  const searchDirs = new Set([MEDIA_DIR]);

  // Also search the video file's directory
  const videoPath = findVideoFile();
  if (videoPath) {
    searchDirs.add(path.dirname(videoPath));
  }

  // Also check MEDIA_DIRS env var
  for (const dir of (process.env.MEDIA_DIRS || '').split(';').filter(Boolean)) {
    searchDirs.add(dir);
  }
  ...
  for (const dir of searchDirs) {
    ...
    const srtFile = files.find(f => {
      ...
      if (epMatch) {
        ...
        if (altMatch && parseInt(altMatch[1]) === s && parseInt(altMatch[2]) === e) return true;
      }
      // Match by episode ID substring
      if (f.toLowerCase().includes(EPISODE_ID.toLowerCase())) return true;
      // Match any SRT in the same directory as the video (if only one SRT exists)
      return false;
    });

    if (srtFile) {
      log(`  SRT found: ${path.join(dir, srtFile)}`);
      const content = fs.readFileSync(path.join(dir, srtFile), 'utf-8');
      return parseSrtContent(content);
    }
  }

  // Last resort: if video directory has exactly one SRT, use it
  if (videoPath) {
    const vDir = path.dirname(videoPath);
    const srts = fs.readdirSync(vDir).filter(f => f.toLowerCase().endsWith('.srt'));
    if (srts.length === 1) {
      log(`  SRT found (only file in video dir): ${srts[0]}`);
      const content = fs.readFileSync(path.join(vDir, srts[0]), 'utf-8');
      return parseSrtContent(content);
    }
  }
