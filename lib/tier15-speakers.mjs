...
async function main() {
  ...
  for (const sc of scenes) {
    for (const sh of (sc.shots || [])) {
      ...
      for (const dlg of shotDialogue) {
        ...
        // Extract character name from subject (may contain multiple characters)
        ...
        if (!subject.includes(',') && !subject.toLowerCase().includes(' and ')) {
          const charName = subject.replace(/^(Captain|Commander|Lt\.|Lieutenant|Dr\.|Doctor|Counselor|Ensign|Chief)\s+/i, '').trim();
  ...
  for (const { sp, votes } of sortedSpeakers) {
    ...
    if (sorted.length > 0) {
      ...
      speakerLabels[sp] = sp; // Keep original label
      log(`  ${sp} → ${sp} (no confident match)`);
    }
  }
